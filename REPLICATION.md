...
`make prepare-data` also downloads the Natural Earth 1:10m disputed-boundary line theme used only for manuscript cartography. DataMeet Census-2001 districts remain the sole source for district fills, dissolved state boundaries, and the empirical map exterior; Natural Earth contributes only India-related claim-line annotations. The required shapefile components are registered with exact byte sizes and SHA-256 digests, so a changed upstream release is surfaced instead of silently changing the paper's boundary annotations.
