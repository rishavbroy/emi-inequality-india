...
`make prepare-data` also downloads the Natural Earth 1:10m breakaway/disputed-area polygon theme used only for manuscript cartography. `data/metadata/map_disputed_areas.csv` explicitly selects Aksai Chin, Azad Kashmir, Gilgit-Baltistan, Shaksam Valley (displayed as the Trans-Karakoram Tract), and Siachen Glacier. The lineage-geometry builder separately retains DataMeet's noncanonical 99/99 "Data Not Available" feature as `data/processed/geography/district_2001_map_scaffold.gpkg`. Public maps union that source-native Jammu-and-Kashmir scaffold with the five selected Natural Earth polygons, subtract the same combined display mask from rendered canonical districts, and redraw it as the disputed/no-district-estimate class. Map state identity is recovered from every canonical Census-2001 district ID rather than from analysis-panel coverage; all 593 districts are dissolved by state and hole-free exterior rings provide ordinary solid state/external outlines. The disputed/no-estimate class owns its own perimeter: a state whose interior overlaps that display class does not contribute a competing exterior outline, while unaffected neighboring states still supply their ordinary interstate border from the Census geometry. This avoids reconstructing a second seam from independently digitized Census and Natural Earth edges. The canonical 593-district GeoPackage remains the sole analytical geography, so joins, samples, spatial weights, and estimates are unchanged. The required Natural Earth 5.1.1 shapefile components are registered with exact byte sizes and SHA-256 digests, so an upstream change cannot silently alter the figures.
