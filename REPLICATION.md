...
The poster document loads project images in `typst-show.typ` before passing the resulting content to the poster function. Keeping that function in `typst-template.typ` avoids a separate package root and lets project resources resolve in the generated document. The template uses three explicit content columns, and `render_poster_pdf()` rejects any result that is not a single-page PDF.
