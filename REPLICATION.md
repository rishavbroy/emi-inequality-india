...
After `plfs_2017_18` materialization is `ready`, the extended labor audit reads only the contracted F1 columns, applies the official annual multiplier rule from the bundled PLFS 2017-18 README, and constructs the documented first-visit person identity. Preferred geography uses the reviewed 2017-18 `primary_source_crosswalk`; the stricter deterministic `conservative_source_crosswalk` is run separately as a sensitivity. Unrestricted population-allocation rows are not treated as resolved PLFS geography. The audit writes the preferred `plfs_2017_18_*` diagnostics/outcomes and parallel `plfs_2017_18_conservative_*` files so coverage, support, and estimates can be compared directly.
