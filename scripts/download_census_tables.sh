...
manifest="${1:-data/metadata/census_2001_download_manifest.tsv}"
...
awk -F '\t' '
  ...
  seen_path[$3]++ { printf "Duplicate destination on row %d: %s\n", NR, $3 > "/dev/stderr"; bad = 1 }
  seen_url[$4]++ { printf "Duplicate URL on row %d: %s\n", NR, $4 > "/dev/stderr"; bad = 1 }
  END { exit bad }
' "$normalized_manifest"
