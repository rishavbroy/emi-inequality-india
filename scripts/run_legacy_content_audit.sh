...
Rscript -e '
if (file.exists("outputs/diagnostics/build/target_warnings.csv") && file.info("outputs/diagnostics/build/target_warnings.csv")$size > 0) {
  warnings <- utils::read.csv("outputs/diagnostics/build/target_warnings.csv", stringsAsFactors = FALSE)
  if (nrow(warnings)) stop("target_warnings.csv is non-empty", call. = FALSE)
}
if (file.exists("outputs/diagnostics/build/target_meta_after_strict_run.csv")) {
  meta <- utils::read.csv("outputs/diagnostics/build/target_meta_after_strict_run.csv", stringsAsFactors = FALSE)
