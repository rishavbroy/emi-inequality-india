...
output_table_helper_chunk <- function() {
  c(
    ...
    "  # Keep captions as plain text. Raw LaTeX line-break helpers inside",
    "  # kable captions are fragile in Quarto excerpt renders and can be",
    "  # escaped into invalid TeX.",
    ...
    "  empty_rest <- if (ncol(df) > 1L) apply(df[-1], 1, function(x) all(is.na(x) | !nzchar(as.character(x)))) else rep(TRUE, nrow(df))",
    ...
    "    data.frame(label = as.character(df[[1]][[group_idx[[i]]]]), start = start, end = end, stringsAsFactors = FALSE)",
    "  })",
