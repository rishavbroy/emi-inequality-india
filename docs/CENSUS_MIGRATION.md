...
The same common sample now also feeds the canonical single-specification IV estimator used by the broader weak-IV diagnostics. Sample construction, reduced forms, multiplicity adjustment, weak-IV estimation, and Anderson-Rubin grid handling are implemented once in `R/diagnostics/posttreatment_mechanism_inference.R` and reused by migration and other Census mechanism families. For each registered migration outcome and each of the six candidate scalar-IV designs, the module reports clustered 2SLS coefficients for scale, the Montiel Olea-Pflueger effective-F diagnostic, the reduced-form joint test, and Anderson-Rubin inference. Anderson-Rubin confidence sets are constructed with the same grid implementation used elsewhere in the project.
