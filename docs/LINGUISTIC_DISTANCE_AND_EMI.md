...
`data/raw/cognates/dyen_kruskal_black_1997.txt` is parsed from its documented fixed-width card representation. Pairwise percentages are `cognate / (cognate + not cognate)`; missing and doubtful judgments are excluded. The parser is required to reproduce Shastry's reported percentages with Hindi after rounding to one decimal: Bengali 64.1, English 14.6, Gujarati 64.6, Kashmiri 42.4, Marathi 56.4, Nepali 64.2, and Punjabi 74.5.
