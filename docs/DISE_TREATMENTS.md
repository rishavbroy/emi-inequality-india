...
Total elementary enrolment follows one hierarchy in every workbook generation: the sum of grade-I-VIII enrolment fields is preferred when available, the workbook's direct `ENRTOT` field is used only when grade totals are unavailable, and the government-plus-private management total is the final fallback. This matches the baseline construction and prevents a malformed later-year `ENRTOT` field from silently overriding internally additive grade counts. The reader retains the grade, direct, and management totals plus their agreement ratios as source-QA fields; Census-2001 child-district aggregation sums those candidate counts first and then reapplies the same hierarchy.
...
The later raw School/Teacher sheets are not yet treated as a comparable 2008-09 through 2015-16 mechanism panel. Inspection against the published report cards reveals district-row alignment failures (for example, 2008-09 Mahe's raw teacher total is attached to a Pondicherry-scale value) and cross-year field-definition discontinuities (including the 2009-10 girls'-toilet series). Dynamic school-quality estimation is therefore deliberately deferred until these outcomes are reconstructed from the published district report cards or otherwise validated year by year. The diagnostic outputs retain the registry and predetermined baseline associations, while the dynamic summary records this deferred status instead of reporting misleading coefficients.

Historical Teacher sheets contribute government-plus-private teacher counts; the 2014-15 and 2015-16 summary sheets expose explicit total-teacher fields. Baseline Teacher sheets are read through a separate mechanism target and merged only after the baseline EMI treatment reader completes, so mechanism availability cannot make the core baseline treatment target fail. Single-teacher-school and girls'-toilet counts use the archived total fields (`tch1_school` / `gtoilet_sch` in the older schema and `stchtot` / `sgtoiltot` in 2014-15 and 2015-16). Facility shares become `NA`, rather than being capped, when a reported count exceeds total schools.

Mechanism diagnostics use the preferred scalar linguistic-distance measure. Predetermined school-quality associations are reported separately for 2005-06 and 2006-07 with state fixed effects and state-clustered inference. No later school-quality trajectory is currently estimated from the raw archive; the registry marks it `deferred_report_reconstruction` until comparable publication-derived measures are available. Later-only amenities such as electricity and computers remain excluded because they do not have a comparable baseline. DISE/UDISE publications report single-teacher-school and girls'-toilet availability as school-system indicators, which makes the published district cards the natural source for the next reconstruction phase.
