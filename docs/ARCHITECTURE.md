...
### Appendix D publication boundary

Appendix D is a presentation layer over already-registered local-development analyses. Migration, housing/assets, Economic Census, household-capacity, social-group schooling, labor, and spatial estimators remain owned by their domain modules; `R/output/appendix_local_development_exhibits.R` only validates and reshapes those objects. The conservative PLFS lineage analysis and ST-concentration heterogeneity are core-owned because they are final-paper sensitivity exhibits, while extended mode continues to persist the forensic CSV bundles. D8 also reuses the three canonical rendered district maps and the same rook-contiguity Moran diagnostics used elsewhere; it does not rebuild geometry, recompute spatial weights, or introduce an appendix-only mapping path. The older `collage_main_maps` and `collage_iv_region_maps` outputs remain transitional render dependencies only because the legacy `paper/paper.qmd` still cites them; they should be deleted when that manuscript is migrated to the new exhibit architecture, rather than carried forward as parallel final-paper exhibits. This avoids recomputing parallel appendix-only models and keeps timing language descriptive for 2001-11 and 2005-13 windows that overlap 2007-08 schooling measurement.
