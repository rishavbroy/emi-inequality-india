...
Census 2011 HL-13 structural-durability counts reuse the shared count/geography path as follow-up-only measures. Census-2001 H-04 Appendix (`PC01_H04a`) is registered for acquisition as `H04A` but remains outside the reader until its raw workbook denominator and structural accounting are validated. Longitudinal durability therefore remains blocked on source inspection rather than being imputed from H-03 materials.
