...
The subsequent paper-design review reopened only a deliberately bounded set of existing-data questions whose answers bear directly on the proposed opportunity/access/conversion narrative. Phase 1 adds three such pieces inside existing architecture: (i) three Hindi-belt sample restrictions in the C-17 behavioral registry, using the same Shastry distance definitions; (ii) descriptive NSS social-group access gaps by sex and by rural/urban sector, with four predeclared schooling margins and no sex-by-sector cube; and (iii) a six-cell 2022 schooling-to-welfare conversion-gradient family crossing all-child EMI and private EMI with three predetermined Census-2001 moderators. The conversion interactions are descriptive, state-clustered, and Holm-adjusted. The 1991 ST-language diagnostic remains nonempty (352 exact ST-16 validated districts, 21 speaker-count mismatches, 43 ST-16-unavailable districts, and 5,913 preferred language cells). The extended-audit gate requires these paper-priority outputs explicitly, so later runs cannot silently omit them. Further additions remain conditional on what the paper blueprint actually needs rather than reopening a general specification search.
