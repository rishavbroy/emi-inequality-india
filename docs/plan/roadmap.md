...
Next, inspect the realized district estimates and their denominator-specific support/uncertainty, then decide and predeclare whether the NSS64 near-treatment exercise enters the historical balance/validity family or a causal mechanism family. After that, add NSS 2009-10 through the same survey interface, followed by later PLFS waves so near-treatment structure remains separate from long-run labor outcomes. Wage outcomes should wait for the weekly-status/earnings source rather than being inferred from the usual-activity block.
