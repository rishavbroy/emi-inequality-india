...
The historical `main` Census-2001 vector is retained as a compact benchmark, not as a uniquely theory-preferred causal adjustment set. Response-definition robustness is now complete, so the finite control-strategy family comparing geography-only, compact-2001, and compact-2001-without-human-capital specifications under region/state FE is activated as the next isolated axis. Keep the finite proxy-parameterization family and the already registered 1991-vintage robustness separate; do not automatically cross these with every response, treatment, and instrument definition.
