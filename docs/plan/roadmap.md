...
Specification governance is now consolidated at the execution boundary. Public headline models are declared by `public_iv_specification_registry()` through the same canonical IV row contract used by newer analyses, while their longstanding model names and formula-based downstream table/report interface are preserved by the thin `iv_specification_formulas()` adapter. `analysis_design_registry` now inventories those public designs alongside the newer families. A separate candidate-design ledger records implemented, partially implemented, unimplemented, and explicitly inadmissible robustness families without estimating them automatically. It identifies the bounded six-design scalar-IV consumption robustness family separately from the rejected endpoint-by-full-diagnostic Cartesian search, and records alternative welfare outcomes, the enrolled-child intensive-margin treatment, and a dedicated 1991 predetermined-adjustment comparison as candidates requiring pre-registration before execution. Do not expand any of these families after coefficient inspection merely because the ledger can represent them.
