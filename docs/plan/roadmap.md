...
The realized NSS64 estimates are now treated as a registered near-treatment reference, not as predetermined balance evidence or a long-run causal mechanism: the survey field period overlaps the 2007-08 EMI measurement window. NSS66 is the early post-treatment labor wave. Its DDI and proprietary `.Nesstar` container are registered and validated; the generic metadata-driven standard-conversion boundary, F4/F5/F6 canonical adapter, same-round reviewed-lineage bridge, and shared NSS labor estimator are implemented. The materialized 459,784-person source has passed the real canonical/source/lineage gate, with all posted weights positive and about 97.9 percent of persons resolving through the deterministic same-round lineage. The four predeclared usual-activity outcomes are therefore now activated through the shared estimator whenever materialization is present, retaining denominator-specific support and the unchanged 5-FSU/Kish-100 preferred rule. Next, inspect the realized NSS66 district-support/outcome diagnostics and then move to later PLFS waves through the same person/design/outcome interface so early and long-run labor responses remain distinct. Wage outcomes should wait for the appropriate weekly-status/earnings source rather than being inferred from usual-activity fields.
