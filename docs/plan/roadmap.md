...
The final planned empirical addition before paper architecture is implemented as a compact, explicitly descriptive OLS bridge from observed schooling margins to later consumption. Enrollment, EMI conditional on enrollment, all-child EMIE, public EMI, and private EMI enter one at a time against the four registered 2022--24 long-run real-mean-MPCE ANCOVA/change specifications under the existing unadjusted / region-main / state-main ladder. Each treatment/outcome pair uses common support across the three columns, ANCOVA retains its registered 2004--05 baseline, and inference is state-clustered with Holm summaries. These associations remain explicitly non-causal and no multiple-schooling-margin pseudo-structural regression is estimated. The empirical roadmap is now closed: the full public audit realizes the bridge and a nonempty 1991 ST-language diagnostic (352 exact ST-16 validated districts, 21 speaker-count mismatches, 43 ST-16-unavailable districts, and 5,913 preferred language cells). The extended-audit gate now requires the final inequality/mechanism milestone outputs explicitly, so a future run cannot silently declare completion after dropping one of these artifacts. The next work is paper architecture and result selection rather than another diagnostic family.
