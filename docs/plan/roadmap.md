...
The historical `main` Census-2001 vector is retained as a compact benchmark, not as a uniquely theory-preferred causal adjustment set. The finite control-strategy family comparing geography-only, compact-2001, and compact-2001-without-human-capital specifications under region/state FE is now complete. The finite proxy-parameterization family is activated as the next isolated axis, while the already registered 1991-vintage robustness remains separate. Do not automatically cross either with every response, treatment, and instrument definition.
