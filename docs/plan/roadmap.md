...
The final planned empirical addition before the first paper-architecture pass was a compact, explicitly descriptive OLS bridge from observed schooling margins to later consumption. Enrollment, EMI conditional on enrollment, all-child EMIE, public EMI, and private EMI enter one at a time against the four registered 2022--24 long-run real-mean-MPCE ANCOVA/change specifications under the existing unadjusted / region-main / state-main ladder. The bridge now uses one complete-case district sample per welfare estimand across **all five schooling margins** and the full adjustment ladder, so comparisons among schooling margins cannot be driven by treatment-specific support. ANCOVA retains its registered 2004--05 baseline, inference is state-clustered with Holm summaries, and no multiple-schooling-margin pseudo-structural regression is estimated.

The subsequent paper-design review reopened only a deliberately bounded set of existing-data questions whose answers bear directly on the proposed opportunity/access/conversion narrative. That P0--P2 empirical tranche is now complete inside existing architecture: (i) three Hindi-belt sample restrictions in the C-17 behavioral registry; (ii) descriptive NSS social-group access gaps by sex and by rural/urban sector, with four predeclared schooling margins and no sex-by-sector cube; (iii) a six-cell 2022 schooling-to-welfare conversion-gradient family crossing all-child EMI and private EMI with three predetermined Census-2001 moderators; (iv) the two-cell EC05 IT-opportunity heterogeneity check; (v) the bounded 2001--2011 household-capacity trajectory synthesis; (vi) common-support schooling-to-welfare comparisons; and (vii) the single Hindi-belt restriction of the skilled recent-work-migrant reduced form. The conversion interactions are descriptive, state-clustered, and Holm-adjusted; the migration restriction changes only the geographic sample of the preferred reduced form. The 1991 ST-language diagnostic remains nonempty (352 exact ST-16 validated districts, 21 speaker-count mismatches, 43 ST-16-unavailable districts, and 5,913 preferred language cells). The extended-audit gate requires the paper-priority outputs explicitly, so later runs cannot silently omit them. No further empirical family is planned before the paper blueprint identifies a specific missing estimand.

### 4. Post-roadmap architecture and imperfect-IV consolidation

The empirical acquisition roadmap is closed, and the two post-roadmap consolidation tasks are complete. The cross-family analysis-design ontology is authoritative for the estimating families needed by the current paper rather than lagging newer diagnostics. Schooling-to-welfare OLS, NSS-64 social-group gap regressions, ST-concentration heterogeneity, and the validated 1991 ST-language regressions now declare canonical family specification grids and project those same rows into `analysis_design_registry`; no parallel per-family specification CSVs are added solely for bookkeeping. The linguistic-instrument part of that separation is now implemented: distance
...
used by existing analyses. The canonical construct layer is now activated by promoting `variable_dictionary.csv` from a descriptive file to the tracked authority for common panel variables and by projecting source-specific C-17, DISE, modern consumption-welfare, migration, housing, Economic Census, labor, ST-language, and social-group constructs into the same reviewer-facing inventory without surrendering source-specific dimensions. Modern welfare constructs are keyed by survey vintage plus welfare concept rather than by generated regression-column names, and the schooling-to-welfare bridge derives shared treatment labels/roles from the canonical registry. The cross-family design registry now carries explicit construct references alongside executable model strings, including endpoint/baseline welfare IDs and multi-instrument component IDs; ambiguous repeated storage names must be resolved by the source family instead of first-match lookup. The broader admissibility layer now has an explicit decision contract: every candidate family carries an `admissibility_reason`, admissibility must agree with `execution_policy`, and canonical IV/design rows separate `control_strategy_id` from `control_parameterization_id` while exposing functional form independently of the estimand label. This makes geography-only, compact observed-adjustment, pathway-robustness, and alternative measurement choices queryable without treating them as one undifferentiated `main`/`expanded` axis. The remaining ontology normalization is now closed at the level needed for new inference work: the four fixed-effect IDs were already canonical, and the cross-family registry now projects legacy estimator/inference/sample strings onto estimation scope, covariance, weak-ID procedure, multiplicity, support policy, and complete functional-form fields. The compatibility strings remain intact, but unknown methodological vocabulary fails closed at the registry boundary. No Cartesian search is generated.

The imperfect-IV consolidation is likewise complete: weak-IV results report the justified information needed before adding another point estimator. Anderson--Rubin confidence-set topology is now classified explicitly so zero rejection, sign identification, disconnection, and grid truncation cannot be conflated. The first bounded inferential addition is now implemented: the four registered modern HCES ANCOVA/change designs receive a Conley--Hansen--Rossi-style bounded direct-effect sensitivity analysis using state-clustered Anderson--Rubin inversion. Exact exclusion nests the existing AR result; reduced-form-scaled symmetric and same-sign supports are labeled fragility calibrations rather than substantive priors, and the generic method accepts externally justified fixed gamma bounds. The second bounded inferential addition is now implemented: Masten--Poirier exclusion-based falsification-adaptive sets summarize the disagreement among the three registered five-share systems under region-main and state-main adjustment. Each share serves as the sole excluded instrument in turn while the other shares are controls; all five constituent estimates and their conditional first-stage diagnostics are retained, and the FAS is their min--max interval. Weak constituents are flagged rather than thresholded away, because the FAS relevance condition is a population assumption and a sample-F screen would create a data-selected estimand. This closes the two imperfect-IV additions prioritized in the architecture review. Nevo--Rosen bounds remain secondary and assumption-indexed. Balke--Pearl discretization, informal OLS-versus-IV bias ranking, mechanical LIML duplication in just-identified designs, and an estimated unobservable bias ratio remain non-goals.
...
The final current-data Shastry comparison controls are now implemented and reviewed. The Hindi-belt indicator materially attenuates the no-FE first stage but leaves the already-weak six-region first stage essentially unchanged. The Census-2001 age-5-19 control is even less consequential: on the same 573-district support the excluded-instrument F changes from about 14.29 to 13.61 without geographic FE and from about 3.238 to 3.227 with six-region FE. These checks therefore reinforce the existing conclusion that conditional relevance, not omitted child-population scale, is the binding issue. No admissible executable current-data candidate remains apart from items explicitly requiring unavailable/deferred data or marked do-not-estimate.

## Response-2 follow-on governance
