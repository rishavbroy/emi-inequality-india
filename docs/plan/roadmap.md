...
used by existing analyses. The canonical construct layer is now activated by promoting `variable_dictionary.csv` from a descriptive file to the tracked authority for domain, vintage, denominator/universe, analysis stage, role, preference, causal-status, and alternative/comparable semantics. Source-specific C-17 and DISE registries project into the same reviewer-facing construct inventory without surrendering their source-specific dimensions, and the schooling-to-welfare bridge now derives shared treatment labels/roles from the canonical registry. The next ontology step is the broader admissibility ledger that similarly separates controls, fixed effects, functional form, estimator, inference, and sample rules without generating a Cartesian search.
