...
The realized NSS64 estimates are now treated as a registered near-treatment reference, not as predetermined balance evidence or a long-run causal mechanism: the survey field period overlaps the 2007-08 EMI measurement window. The NSS66 DDI and proprietary `.Nesstar` container are now registered and validated as the early post-treatment source. Next, convert the NSS66 container with the standard `nesstar-converter` tool, inspect the converted F4/F5/F6 tables, and extend the shared labor estimator to 2009-10. Later PLFS waves should then enter through the same person/design/outcome interface so early and long-run labor responses remain distinct. Wage outcomes should wait for the appropriate weekly-status/earnings source rather than being inferred from usual-activity fields.
