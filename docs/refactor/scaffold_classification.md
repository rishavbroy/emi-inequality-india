...
Generic scaffold `@return` comments have been removed from active R files. Remaining pass-through helpers are classified here so they are not mistaken for completed semantic ports; each must either be implemented during the district-harmonization port or deleted once downstream calls are removed.
