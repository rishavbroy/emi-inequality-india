...
PLFS district identity uses the same `state + NSS-region digit + district` code convention as the reviewed NSS 2017-18 source roster. Preferred PLFS geography now reuses the repository's existing `primary_source_crosswalk`, which is already the adjudicated production bridge for 2017-18: deterministic rows plus accepted single-target primary upgrades. The stricter `conservative_source_crosswalk` is estimated separately as a deterministic sensitivity. Unrestricted `population_allocation` rows are not accepted by the PLFS preferred bridge.

Realized F1 inspection finds 655 source district identities. The reviewed primary bridge resolves 614 districts and about 94.85% of sampled persons; the conservative deterministic bridge resolves 446 districts and about 68.17%. Within the primary bridge, deterministic mappings retain `resolved_reviewed_deterministic` provenance and only accepted single-target upgrades receive `resolved_reviewed_primary`; the underlying lineage basis is carried onto person rows.

The realized four-outcome sensitivity closes the 2017-18 geography gate. The primary bridge yields 565 estimable Census-2001 targets versus 420 under the conservative bridge. Preferred 5-FSU/Kish-100 coverage is 469 versus 341 targets for LFPR/employment, 257 versus 183 for unemployment, and 239 versus 171 for regular-salaried share. Among the 420 targets common to both variants, only 15 cells per outcome change at all; estimate correlations range from about 0.987 to 0.997. Thus accepted primary upgrades primarily recover geographic coverage rather than broadly changing already-covered district estimates. `plfs_2017_18_variant_comparison.csv` records this comparison directly. PLFS 2017-18 is therefore the completed principal `long_run_post` usual-status labor wave; later PLFS waves should be added only for a predeclared dynamic/robustness purpose, not to search across endpoints.


## Registered local-development labor family

The design-based district tables remain broader than the causal mechanism family. Before inspecting any labor IV coefficients, the registered paper-facing family is frozen to the two age-15+ outcomes with the strongest and common denominator support: labor-force participation and employment. Unemployment and regular-salaried composition remain secondary district outcomes because their labor-force/employed denominators materially reduce preferred support; NSS64 remains a near-treatment reference and is not entered into post-treatment IV mechanism inference.

NSS66 supplies the earlier labor wave and PLFS 2017-18 supplies the later labor wave. These are reduced-form local-development outcomes, not identified EMI mechanisms. The PLFS conservative deterministic geography is estimated as a robustness sample, not a separate hypothesis family. All three model sets reuse the shared post-treatment mechanism inference layer already used by Census and Economic Census mechanisms: the registered IV design grid, one common support sample for LFPR/employment within each wave/variant, state-clustered reduced forms and 2SLS, effective-F diagnostics, Holm correction within specification, and Anderson-Rubin confidence sets. The labor estimator does not create another IV implementation.

Reviewer-facing outputs use wave prefixes such as `nss66_mechanism_*`, `plfs_2017_18_mechanism_*`, and `plfs_2017_18_conservative_mechanism_*`. The common-support source is constructed only from district cells that passed the predeclared 5-FSU/Kish-100 support rule; thin unemployment and regular-salaried cells therefore cannot shrink the LFPR/employment paper-facing sample merely because they are part of the broader descriptive registry.
