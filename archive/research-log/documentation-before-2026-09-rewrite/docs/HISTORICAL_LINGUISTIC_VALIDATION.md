...
The support rows in `vanneman_pretrend_geography_support.csv` compare strict
one-to-one and broader historical geographies for the full sample and each
distance-support layer. They are the decision gate for any further
constant-boundary amalgamation: merger components should only be implemented if
the realized support gain after historical-parent aggregation and broad
Helms-Lim coverage remains material. Keeping this information in the aligned
geography comparison avoids persisting the same support table twice.
...
## Scheduled Tribe language-acquisition diagnostic

The Census-1991 ST language sources are now activated as a bounded pre-treatment mechanism diagnostic. The implementation intentionally uses the **district ST-17 workbooks** for bilingual/trilingual behavior because the state-level ST-17 files contain state aggregates rather than district observations; the companion **ST-16 district rows** are used only as an independent denominator/coverage gate. ORGI describes ST-17 as reporting, for each Scheduled Tribe group and mother tongue, speakers of each subsidiary language; its published columns separately report second- and third-language counts. ST-16 reports Scheduled Tribe population by mother tongue with rural and urban counts. Historical state identities remain on their 1991 geography.

The parser keeps only the `All Scheduled Tribes` block. Rural and urban counts are pooled before shares are computed. Blank workbook cells are normalized to empty strings at the reader boundary because `readxl` represents the table's intentionally blank separator cells as `NA`; blanks are therefore data-layout structure, not block terminators. Printed third-language layout labels are recognized by their published semantic prefixes (`Total`, `Not speaking`, `Third language`, and sex headings), case-insensitively after label normalization, so harmless wording variants cannot turn table furniture into language observations. Some published ST-17 workbooks repeat the same bilingual, second-language, or third-language category on adjacent lines. These repetitions are not independent people: the parser therefore takes the maximum repeated count within a mother-tongue/residence/second-language category and within a second-language/third-language pair, then enforces the published identities `monolingual + bilingual(including trilingual) = mother-tongue speakers` and `English or Hindi second+third <= bilingual speakers`. This prevents duplicate printed lines from being double-counted while remaining fail-closed on inconsistent counts.

The preferred diagnostic is deliberately conservative. ST-16 is a hierarchical published workbook: state, district, and Scheduled-Tribe labels are printed once for the mother-tongue rows beneath them. The reader therefore carries only those grouping labels downward before filtering the `All Scheduled Tribes` block; mother tongues and rural/urban counts are never filled. Structural table labels are matched with the repository's canonical key normalization rather than language title-casing, so capitalization, spacing, and the published historical `Sheduled` spelling variant cannot change sample membership. ST-17 likewise admits a mother-tongue total only when the printed `Total No. of <language> speakers` label contains a nonblank language name, so the generic `Total No. of speakers` table row cannot become an empty-language observation. A district enters only when its complete ST-17 mother-tongue speaker vector reconciles exactly to the independently published ST-16 district counts and the mother tongue maps to the frozen historical Shastry distance basis. Validation mismatches and states without an acquired ST-16 companion remain visible in the coverage outputs and are excluded rather than repaired by proportional allocation or fuzzy language matching. In the supplied source set, the source inspection found exact ST-16 agreement for 352 districts, ST-16 discrepancies for 21 Assam districts, and no acquired ST-16 companion for Bihar; the generated diagnostic recomputes those statuses rather than hard-coding them. The diagnostic fails closed if exact ST-16 coverage or the preferred mapped language sample ever collapses to zero, because an empty estimate table is a source-contract failure rather than a substantive null result.

The diagnostic estimates population-weighted mother-tongue-level relationships between historical Shastry distance and (i) English second/third-language acquisition, (ii) Hindi second/third-language acquisition, (iii) their difference, and (iv) overall bilingualism. Models include 1991 state fixed effects and state-clustered HC1 covariance. A second sample applies the historical 1991 equivalent of Shastry's Hindi-belt definition; pre-2000 state identities are retained (for example, Jharkhand remains Bihar and Chhattisgarh remains Madhya Pradesh). These estimates are descriptive mechanism evidence, not a new instrument, control family, or causal outcome family.
