> **Historical snapshot.** This is the pre-rewrite replication/status document retained during the documentation reorganization. It is not an active execution guide; use [`../../../REPLICATION.md`](../../../REPLICATION.md).
...
The R package dependencies are declared in [`DESCRIPTION`](../../../DESCRIPTION), and [`renv.lock`](../../../renv.lock) records exact package versions. Spatial packages such as `sf` and `spdep` may also require GDAL, GEOS, PROJ, and `pkg-config` system libraries. PDF text checks use Poppler's `pdftotext` executable. Application-sample section selection requires Quarto 1.4 or newer because it runs at the supported `post-quarto` filter stage; the conference poster requires Quarto 1.9.18 or newer for its Typst custom format. The format follows Quarto's standard two-part template structure: `typst-template.typ` defines the poster function and `typst-show.typ` passes document metadata and body content to it. On macOS, these are commonly available through Homebrew as `gdal`, `geos`, `proj`, `pkg-config`, and `poppler`.
...
`run_full_build.sh` uses [`config/final.yml`](../../../config/final.yml), includes application samples, writes `review.zip`, and leaves the conference poster out unless requested. GitHub Pages publication is separate: the Pages workflow deploys only committed PDFs and does not attempt to reconstruct the empirical results on GitHub-hosted runners. Useful options are:
