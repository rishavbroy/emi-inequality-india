...
# Herein are all 40 code chunks used for the research paper "Escaping Inequality in India." They are organized like so:
...
# Chunk 8: Participation model: Is missingness associated with observed covariates
  # All variables: Variables/Regions with NAs
  # Correlation matrix
  # Logistic per missing variable
...
# Chunk 17: Match districts: Define joining functions
  ...
  # merge_dfs_into_tracker(dfs, tracker, years_of_interest, flag)
...
# Chunk 20: Match districts: Diagnose errors
  # unmatched_rows(dfs, tracker, years_of_interest)
...
# For easier navigation, use one of the following:
# 1) Search (e.g., using ctrl/cmd+F) for specific chunk, section, and/or function names.
# 2) Open the table of contents or document outline in your PDF viewer's navigation pane.
...
options(tinytex.verbose = TRUE) # Leave uncommented to help troubleshoot when knitting
...
# Recall: Long filenames were used so that updated versions of data files could be downloaded and easily plugged in.
# ---
...
# start.time <- Sys.time()
# 35 files will be read in. Preallocate list to store their cleaned data
files <- list.files(
  data_folder,
  pattern = "^PC01_C16_\\d{2}\\.xls$",
  full.names = TRUE
)
...
for (i in seq_along(files)) {
  ...
  if(length(state_val) == 0) {
    ...
    state <- state_val[1] %>% 
      ...
      str_remove("^State -\\s*") %>%
      # Remove four-digit suffix code
      str_remove("\\s*\\d{4}$") %>%
      # Remove leading/trailing spaces. Squish internal spaces
      str_squish() %>%
  ...
  temp <- temp %>% 
    filter(str_detect(as.character(mother_tongue_code), "0$"),
           str_detect(area_name, "^District - "))
  
  # Create "district" and "state" columns
  temp <- temp %>% 
    mutate(
      # Create "district" by cleaning "area_name"
      district = area_name %>% 
        str_remove("^District -\\s*") %>%
        str_remove("\\s*\\d{4}$") %>%
        str_squish() %>% 
        # Remove all trailing non-letters
        str_replace("[^[:alpha:]]+$", ""),
      
      # Create "state" via extracted state name
      state = state
    )
  
  # Clean "mother_tongue" column
  temp <- temp %>% 
    mutate(mother_tongue = mother_tongue %>% 
             str_remove("^\\d{1,3}\\s+") %>% # Remove leading 1-3 digits and a space
             str_to_title()) # Title case
  
  # Save cleaned df in list
  result_list[[i]] <- temp
}
...
# end.time <- Sys.time()
# end.time - start.time 
# For us, above order (clean files, then bind) was 1.48 secs faster than alternatives.

# Uncomment below to visually verify results
# census01 %>% select(-c(table, tehsil_code, area_name, mother_tongue_code)) %>% group_by(state) %>% distinct(district) %>% View()


# ---

#### District boundaries and regions ####
# From @meyersIndiaOfficialBoundaries2020 via @bhatiaMergingUpdatedDistrictlevel2020 
data_folder <- "District Boundaries 2020"

district_bnds_20 <- st_read(
  file.path(data_folder, "district", "in_district.shp"), 
  quiet = TRUE) %>% 
...
# 16 districts :(
# Fix: To join data from multi-year surveys (e.g., 07-08), we'll need to check for matches across all sampled years at least



# View number of districts each year with the same name despite being in different states
...
keep_list <- lapply(year_suffixes, function(sfx) {
  ...
  temp <- district_tracker %>%
    ...
    summarise(
      n_states = n_distinct(.data[[state_col]], na.rm = TRUE),
      .groups = "drop"
    )
...
n_same_name_districts <- temp %>% 
    ...
    summarize(n = n())
# Between min(n_same_name_districts$n) = 3 and max(n_same_name_districts$n) = 5 districts with the same name in different states in each year of interest
...
# Chunk 8: Participation model: Is missingness associated with observed covariates {.unnumbered}
```{r Participation model: Is missingness associated with observed covariates, message=FALSE, warning=FALSE }
...
temp2 <- model.matrix(~.-1, data = selection_df %>% select(all_of(group_vars))) # -1 removes intercept column
...
sel_corr_mat_all <- cor(
  ...
  use = "pairwise.complete.obs" # Corr found for complete pairs. NA entries allowed. Pearson method.
)

# Second, only for enrolled observations
# Make new df with missingness indicator vars
temp1 <- selection_df %>% 
  filter(enrolled=="Yes") %>% 
  mutate(
...
# Expand factors to set of dummy variables
temp2 <- model.matrix(~.-1, 
                      data = selection_df %>% filter(enrolled=="Yes") %>% select(all_of(group_vars)),
                      na.action=na.pass
                      )
# Compute correlation matrix
...
check_missing_logit_parallel <- function(df, miss_vars, covars, method_p = "BH") {
  ...
  # To fit the binary outcome is.na(m) for each missing var m
  fit_one <- function(m) {
    f <- as.formula(paste0("is.na(", m, ") ~ ", rhs))
    fit <- glm(f, data = df, family = binomial)
    pseudoR2 <- 1 - fit$deviance / fit$null.deviance # Note: No missingness indicator was constant
    gl <- glance(fit) # Currently unused; retained for future diagnostics
    
    # Return tidy df of coefficients and attach missing var's name
    broom::tidy(fit) %>% # Namespace made explicit for cluster workers
      mutate(
        missing_var = m,
        pseudoR2 = pseudoR2,
        nobs = stats::nobs(fit)
  ...
  # Parallelization
  # Cluster with parLapply() if on Windows
  out_list <- if (.Platform$OS.type == "windows") {
    cl <- parallel::makeCluster(max(1, parallel::detectCores() - 1))
    on.exit(parallel::stopCluster(cl), add = TRUE) # Shut down cluster even if errors occur
  ...
  parallel::clusterExport(
    cl,
    varlist = c("df", "rhs", "fit_one"),
    envir = environment()
  )
  parallel::parLapply(cl, miss_vars, fit_one)
  } else { # Fork with mclapply()
    parallel::mclapply(miss_vars, fit_one, mc.cores = max(1, parallel::detectCores() - 1))
  ...
  out <- out %>%
    ...
    mutate(
      p_adj = { # Adjust within each missingness model
        adj <- rep(NA_real_, dplyr::n())
        idx <- term != "(Intercept)" # Exclude intercept
        adj[idx] <- p.adjust(p.value[idx], method = method_p)
        adj
      }
      ) %>% 
    ungroup() 
...
covars <- c("SECTOR", "SEX", "AGE", "HH_SIZE", "RELIGION","SOCIAL_GROUP", "state_0708")

# Run on missing variables defined for all children
logit_missing_results_all <- check_missing_logit_parallel(selection_df, miss_vars_all, covars, method_p = "BH") # "BH" = Benjamini-Hochberg control on false discovery rate. 
# "BY" lets independence or positive dependence assumptions weaken to arbitrary dependence, but has weaker power (i.e., more p-vals above 0.05).

sel_logit_NA_all <- logit_missing_results_all %>%
  group_by(missing_var) %>% # Use name from fit_one()
  summarise(
...
# Run on missing variables defined for schoolgoing children only
logit_missing_results_enrolled <- check_missing_logit_parallel(selection_df, miss_vars_enrolled, covars, method_p = "BH") # "BH" = Benjamini-Hochberg control on false discovery rate. 
# "BY" lets independence or positive dependence assumptions weaken to arbitrary dependence, but has weaker power (i.e., more p-vals above 0.05).
...
# Ex: The "Jhelam Valley" stratum has only one PSU in the data.
...
# Will thus use "average" for handling single-PSU strata (e.g., "Jhelam Valley") when estimating variance
options(survey.lonely.psu = "average")

# Create weighted survey design model
selection_design <- svydesign(
  ...
  # Sampling occurred in two stages: the first-stage units (FSU) were rural villages/urban blocks (FSU_SL_NO), and the ultimate stage units (USUs) were households (HHID)
  
  strata = ~interaction(
    as.character(STATE),
    as.character(STRATUM),
    as.character(SUB_STRATUM_NO),
    drop = TRUE # Nothing dropped yet
    ), 
...
) # Rows with an NA for a probit-relevant variable dropped!
...
# Rows with no NA under probit-relevant variable
needed_vars <- all.vars(f_probit) # = c(probit_vars, "enrolled") 

# Take only complete cases of variables in f_probit
...
lp[comp_cases] <- predict(model_probit_selection,
                         ...
                         type = "link") # to return linear predictors
...
# .Machine$double.eps = 2.220446e-16 on my device
...
autodiff(autodiff = TRUE, install = FALSE) # install=TRUE if self-managing Python install
options(marginaleffects_parallel = FALSE) # Make sure it's off
sel_data <- selection_df[comp_cases, , drop = FALSE]

num_samp <- 200
set.seed(999)
final_draft <- TRUE # Set TRUE when rendering final draft
newdata_ame <- if (final_draft) sel_data else sel_data %>% slice_sample(n = num_samp)
...
mfx_all <- avg_slopes(
  ...
  newdata = newdata_ame,
  wts = "weight",
  vcov = TRUE, # Set FALSE to make this even faster
  type = "response"
)
...
# num_samp <- 2000
# set.seed(999)
# newdata_ame <- sel_data %>% slice_sample(n = num_samp)
# # newdata_ame <- sel_data
# 
# start.time <- Sys.time()
# 
# mfx_all <- avg_slopes(
...
```{r Match districts: Test joining methods, warning=FALSE}
...
evaluate_distances <- function(pairs,
  ...
  if(length(methods) != length(thresholds)) {
    stop("\"methods\" and \"thresholds\" must have the same length.")
  }
  # Ensure chr type
  pairs <- pairs %>%
    mutate(
...
# soundex=0 --> Phonetic variants in anglicization allowed (if coarsely measured)
# qgram = 0 --> Some rearrangements of characters allowed
...
# Template for testing these vs. other methods/thresholds can be found at start of chunk


# Evaluate distances and test joining methods
# colSums(is.na(joined_df))
...
# A. Loop over the methods*thresholds
# B. For each methods*thresholds pair, fuzzy full join all still‑unmatched rows
# C. Return joined, unmatched_df1, unmatched_df2
fuzzy_join_sequence <- function(df1, df2,
  ...
  for(i in seq_along(methods)) {
    ...
    # Keep only pairs (i.e., those with both sides present) and rank them
    matched_i <- full_j %>%
    ...
    # Remove them from the to‑do sets
...
# Default is full join so that this can be used for both matching and diagnostics


# For each df name in input vector:
# A. Extract its "01"/"05"/... suffix
# B. Find the suffix’s district_/state_ columns in joined_df. 
# C. Run the fuzzy join sequence on each column. (Recall justification of fuzzy joining states in Chunk 6: Match districts: Construct district tracking dfs.)
# D. Repeat with the predecessor suffix.
merge_dfs_into_tracker <- function(
  ...
  if (flag) {
    ...
    matched_lists <- lapply(names(dfs), function(x) rep(NA_character_, nrow(tracker)))
    names(matched_lists) <- names(dfs)
  }

  year_suffixes <- substr(as.character(years_of_interest), 3, 4)
  unmatched_list <- vector("list", length(dfs))
  names(unmatched_list) <- names(dfs)

  # 2. Loop over each auxiliary data-frame name
  for (d_name in names(dfs)) {
    df_d <- dfs[[d_name]]
    suffix <- stringr::str_extract(d_name, "\\d{2}$")
    stopifnot(!is.na(suffix))
    ...
    unmatched_list[[d_name]] <- df_unmatched
...
# Errors were identified in Chunk 20: Match districts: Diagnose errors
...
# Helper vectors:
dfs <- list(mother_tongues_01, df0708, df1718, districts_20)
...
temp <- merge_dfs_into_tracker(
  dfs = dfs, tracker = district_timeseries,
  years_of_interest = c("2001", "2011", "2024"),
...
unmatched_df_timeseries <- bind_rows(temp$unmatched, .id = "source")
...
temp <- merge_dfs_into_tracker(
  dfs = dfs, tracker = district_tracker,
...
unmatched_df_tracker <- bind_rows(temp$unmatched, .id = "source")
...
# Errors are fixed in Chunk 18: Match districts: Manually fix errors

# Change dfs for diagnostic purposes, if desired
dfs <- list(mother_tongues_01, df0708, df1718, districts_20)

# Function to extract mismatches:
unmatched_rows <- function(
  ...
  result_list <- lapply(names(dfs), function(nm) {
    out <- merge_dfs_into_tracker(
      dfs = dfs[nm], tracker = tracker,
      years_of_interest = years_of_interest,
      methods = methods, thresholds = thresholds
    )$unmatched[[nm]]
    
    # Add source column of length nrow(out)
    out <- out %>% mutate(source = nm)
    out
  })
...
# Identify errors for each tracker df
...
all_rows <- bind_rows(
  lapply(names(dfs), function(nm) dfs[[nm]] %>% mutate(source = nm)),
  missing_districts_1718 %>% mutate(source = "missing_districts_1718"),
  missing_districts_0708 %>% mutate(source = "missing_districts_0708"),
  district_timeseries %>% mutate(source = "district_timeseries"),
  district_tracker %>% mutate(source = "district_tracker")
  ) %>% 
  mutate(
    state = coalesce(
      !!!select(., starts_with("state_"), -contains("code"))
...
# # Test results under different contiguity measures; code based on @liChapter14Spatial2019
...
# # 4.783471
# # Average neighbor counts are nearly equivalent

# # Test for fastest contiguity measure
# start.time <- Sys.time()
nb_2020 <- joined_df %>%  
  poly2nb(queen = FALSE)
# end.time <- Sys.time()
...
# # Rook contiguity slightly faster

# Binary adjacency matrix. Rook-contiguity neighbor list used for speed, conceptual interpretability
W_2020 <- nb2mat(nb_2020, style = "B", zero.policy = TRUE) 
# W_2020[i,j] == 1 if districts i and j share a border, 0 else
...
first_stage_consumption <- lm(
  ...
  data = joined_df # Complete cases already enforced, so ivreg() and lm() will use the same estimation sample
)
...
joined_stats <- joined_stats %>%
  mutate(
    ...
    )# %>% mutate(across(Min:SD, ~ signif(.x, 3))) # For 3 significant digits
...
# # Num. total columns:
# ncol(X)
# # Recall: If qr(X)$rank < ncol(X), exact collinearity.

# Heuristic: kappa>30 implies severe multicollinearity
X_no_intercept <- X[, colnames(X) != "(Intercept)", drop = FALSE]
keep <- apply(X_no_intercept, 2, sd, na.rm = TRUE) > 0
X_scaled <- scale(X_no_intercept[, keep, drop = FALSE])
kappa_val <- kappa(X_scaled, exact = TRUE)

# # Check for linear dependence
# X_df <- as.data.frame(X_scaled)
# alias(lm(
#   consumption_pct_change ~ .,
#   data = cbind(consumption_pct_change = joined_df$consumption_pct_change, X_df)
# ))

# # Heuristic: VIF > 10 implies problematic collinearity
# vif(lm(
#   reformulate(
...
# Recall: Number+order of rows in 2SLS models = number+order used for spatial weights
listw_2020 <- nb2listw(nb_2020, style = "W", zero.policy = TRUE) # TRUE for, say, islands
...
# # p-vals via Monte Carlo (moran.test() assumes asymptotic normality)
# set.seed(999)
# num_m = 9999
...
## fig-ilo-trends (Figure \@ref(fig:fig-ilo-trends)) {.unnumbered}
```{r fig-ilo-trends, fig.cap="Trends in earnings, labor‐force participation, and unemployment (ILO, 2024).", fig.align="center", out.height='0.9\\textheight', dev='png', dpi=300, fig.pos='p'} 
...
## tab-probit-summary-numeric (Table \@ref(tab:tab-probit-summary-numeric)) {.unnumbered}
```{r tab-probit-summary-numeric, results='asis'}
kbl(
  sel_quant_stats %>% 
  ...
  booktabs=TRUE, 
  longtable=TRUE, 
  label = "tab-probit-summary-numeric", 
  ...
  # # Group district-level aggregate rows into single block
...
## tab-probit-summary-categorical (Table \@ref(tab:tab-probit-summary-categorical)) {.unnumbered}
```{r tab-probit-summary-categorical, results='asis'}
kbl(
  sel_cat_stats %>%
  ...
  booktabs=TRUE, 
  longtable=TRUE,
  label = "tab-probit-summary-categorical", 
...
## tab-probit-ame (Table \@ref(tab:tab-probit-ame)) {.unnumbered}
```{r tab-probit-ame, results='asis'}
...
kbl(
  ...
  longtable = TRUE,
  linesep = "",
  label = "tab-probit-ame",
  caption = "Average Marginal Effects for Enrollment Probit",
  align = c("l", "r", "r")
) %>%
  kable_styling(
  ...
  footnote(
    general = "Data from the 64th round of the NSS, ``Participation and Expenditure in Education'' in 2007–08. All standard errors are design-based, with PSUs clustered and nested within strata.",
    threeparttable = TRUE,
    escape = FALSE
  )
...
## fig-district-maps-main (Figure \@ref(fig:fig-district-maps-main)) {.unnumbered}
```{r fig-district-maps-main, fig.cap="(Clockwise from top left) EMI exposure, consumption growth, pucca (permanent) housing, and household heads with secondary education or more. Data from the 64th round of the NSS 2007-08, ``Participation and Expenditure in Education'' and ``Household Consumer Expenditure.''", out.width="\\textwidth"}

knitr::include_graphics("collage1_map.png")

```

## tab-iv-summary (Table \@ref(tab:tab-iv-summary)) {.unnumbered}
```{r tab-iv-summary, results='asis'}
...
kbl(
  ...
  ## pack_rows, with row indices from joined_stats
  pack_rows("From 2001:", 1, 1) %>%
  pack_rows("From 2007-08:", 2, 22) %>%
  pack_rows("From 2017-18:", 23, 25) %>%
...
## fig-regions-linguistic-distance (Figure \@ref(fig:fig-regions-linguistic-distance)) {.unnumbered}
```{r fig-regions-linguistic-distance, fig.cap="From left to right: regions of India and linguistic distance from Hindi. Data from the 2001 Census of India.", out.width="\\textwidth"}

knitr::include_graphics("collage2_map.png")
```

## tab-first-stage-consumption (Table \@ref(tab:tab-first-stage-consumption)) {.unnumbered}
```{r tab-first-stage-consumption, tab.cap="First-Stage Regression, Linguistic Distance on EMI Exposure", tab.id="tab-first-stage-consumption"}

# Mapping for covariate labels
var_labels <- c(
...
vcov_partialf <- vcovCL(first_stage_consumption, cluster = joined_df$state_20)
...
partial_F_test <- linearHypothesis(
  ...
  vcov. = vcov_partialf,
  test = "F"
) # Wald F-stat with robust vcov + one endogenous regressor "is equivalent to the Kleibergen-Paap statistic" (https://lrberge.github.io/fixest/reference/fitstat.html)
...
# Add custom GOF row for the partial F-stat
...
## tab-second-stage-consumption-iv (Table \@ref(tab:tab-second-stage-consumption-iv)) {.unnumbered}
```{r tab-second-stage-consumption-iv, results='asis', tab.cap='Second-Stage Regression: Consumption Growth on EMI Exposure', tab.id='tab-second-stage-consumption-iv'}
...
## fig-district-carveouts-shifts (Figure \@ref(fig:fig-district-carveouts-shifts)) {.unnumbered}
```{r fig-district-carveouts-shifts, fig.cap="Number of 2001 districts which absorbed a percentage of a 1991 district's population via name change, clean merger, carve-out, or border shift. Data from Kumar \\& Somanathan (2016).", out.width="0.8\\textwidth", fig.align='center'}
