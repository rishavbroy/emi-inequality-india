...
After manual review, copy approved sections into paper.qmd. This working draft
is intentionally excluded from the strict public-paper and application-sample
contracts; render it explicitly with `make paper-new`.
-->
...
Table @tbl-schooling-market-geography collects the institutional-geography evidence that otherwise sits across several diagnostics. Linguistic distance is positively associated with every schooling margin nationally, but the standardized relationship attenuates sharply after region and especially state absorption. State membership explains much more of public EMI than private EMI variation. Finally, DISE and NSS EMI measures remain strongly correlated after state means are removed, so the attenuation is not plausibly explained by NSS measurement error alone.
