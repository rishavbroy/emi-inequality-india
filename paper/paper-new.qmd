...
\end{quote}
First, note the improvements made here over the previous literature. The setting of 2007-2018 may be more relevant today than the settings of similar literature, all of which are closer to the advent of service job offshoring in the 1990s [e.g., @refeque2022a]. Additionally, note that our response variable here is consumption, not wages. India holds a vast number of informal and self-employed individuals, meaning its labor force varies wildly in the degree to which wages reflect material well-being.^[See @shrivastavInformalityIndianLabour2026 and @bahlInformalityEducationoccupationMismatch2024 for the frequency and wage effects of informality respectively. Wages may still be more responsive to EMI in the medium-run window we study, however. See @sec-alty for more.]

Second, we must highlight that unlike much of this literature, we lack household panel data. We will thus follow previous papers [e.g., @martinBigOutSmall2017] and aggregate data at the district level.^[A district in India is akin to a county in the United States, but with an average population of 2 million. @tbl-core-summary summarizes the central district-level constructs used in the paper.] More specifically, we will take cross-sectional data from 2007-08 [@nationalsamplesurveyoffice2008] and 2017-18 [@office2022a], aggregate each at the district level separately, and match districts across the time periods to construct a district pseudo-panel. District-level variables are developed in @sec-iv-model and the district matching method is explained in @sec-distma-spa .

Our empirical methodology will then proceed in two parts. In @sec-heckman, we study the composition of our key "EMI exposure" measure by estimating an individual-level probit of school participation in 2007-08. In @sec-iv, we move to the district level and implement two-stage least squares (2SLS): the weighted average of linguistic distance from Hindi in 2001 will be used to instrument for EMI exposure, onto whose fitted values we regress the change in log person-weighted real consumption.^[Future work may incorporate both education selection and EMI exposure as endogenous regressors in a first-difference (FD) 2SLS design. The district-level aggregates of @tbl-sum-tbl-probit-quant may be able to function as education supply-shifting instruments.] The choice of instrument is discussed in @sec-iv-iv.

Our 2SLS results will, at best, reflect the medium-run local general equilibrium effects of EMI exposure. On one hand, by aggregating away all individual or household-level variation, the ecological fallacy renders our 2SLS estimates uninterpretable at these lower levels. Additionally, school-going children in 2007-08 will still be quite young in 2017-18, and many may still be in school (particularly in more developed districts).^[Cohort-specific measures of EMI exposure may thus be incorporated into future drafts, as may robustness checks that use school-reported data on medium of instruction to construct EMI exposure---with the aforementioned caveat that schools may lie about having EMI [@mathew2011a; @lahoti2019a]. Such surveys may also reveal whether EMI exposure is rigid over time. If so, this could allow us to expand the window of time over which we can interpret our results. Rigidity could also rule out the agglomeration of EMI discussed in @sec-disc as a potential attenuating force behind our results.]

In the preferred specification, the estimated effect of EMI exposure on person-weighted real consumption growth is statistically imprecise (@sec-disc), in line with previous literature on EMI and Indian education quality. Note, however, that our outcome includes households not directly exposed to EMI, meaning the effect we're studying would likely be inherently small regardless. Migration and spatial spillovers may also attenuate our results; evidence which both challenges and supports this is discussed in detail in @sec-spa).
...
\end{aligned}
$$ {#eq-iv-eq}
where $X_d$ contains predetermined Census 2001 controls and $\alpha_s$ denotes Census-2001 state fixed effects.
The scale and dispersion of the paper's central schooling, linguistic, predetermined-capacity, and long-run welfare constructs are summarized in @tbl-core-summary. @fig-schooling-geography places the preferred linguistic condition beside all-child, public, and private EMI exposure on the common Census-2001 district geography; @fig-map2-fig later shows the regional and residual identifying geography.
...
We are currently unable to replicate her justification of the exclusion restriction, however. Her argument centers on a map depicting the geographical balance of her residual variation, made possible despite the distinct regional divide in @fig-map2-fig thanks to state fixed effects. In our case, the state-FE specification has condition number $\kappa =$ `r report_value("kappa")` even though the production pseudo-panel uses unique Census-2001 target districts and passes the lineage validation gates. The remaining instability is therefore an identification problem rather than an artifact of duplicated panel rows: linguistic distance is strongly structured across states, leaving little conditional variation once state fixed effects and predetermined district controls are included. @fig-first-stage-absorption makes that absorption explicit on one common district support. The broad no-FE relationship is sizeable, but excluded-instrument strength collapses when the comparison is restricted within states; adding richer predetermined controls does not restore it.
...
![From left to right: regions of India and linguistic distance from Hindi. District-level data, from the 2001 Census of India.](../outputs/figures/main/collage_iv_region_maps.pdf){#fig-map2-fig fig-pos="H" width="100%"}




Part of this absorption is structural: the States Reorganization Act of 1956 aligned many state lines with linguistic lines, a precedent later states have often followed [@sarmaIntersectionsHeritageMultilingualism2025]. This is why the state-FE first stage is an important stress test rather than a nuisance to be removed. Future work should seek excluded variation that survives within-state conditioning, potentially through time-varying supply or demand shocks and weak-IV-robust estimators rather than dimensionality-reduction methods that change the identifying variation.
...
The results of our first-stage regression, of EMI exposure on linguistic distance, are provided in @tbl-fs-cons. The results of the second-stage regression, where real log consumption growth is regressed onto the fitted values of EMI exposure, are provided in @tbl-cons-iv. All standard errors in both stages are clustered at the state level.
...
render_public_table("../outputs/tables/main/fs_cons.tex", "fs_cons")
```
...
render_public_table("../outputs/tables/main/cons_iv.tex", "cons_iv")
```

Before returning to the instrument, @tbl-paper-schooling-welfare asks a narrower descriptive question: are the schooling margins emphasized in the access analysis associated with later district living standards on common support? The table compares enrollment, conditional EMI, all-child EMI, public EMI, and private EMI using the same state-fixed-effects and predetermined-control specification within each welfare endpoint. English-oriented margins are substantially more positively associated with 2022 consumption than enrollment alone, while the corresponding 2023 relationships are weaker. Because these are conditional district associations rather than experimentally or quasi-experimentally isolated returns, they motivate rather than resolve the identification question.
...
The current state-FE candidate specification uses person-weighted real log consumption growth as the outcome and the predetermined Census 2001 control set; the region-FE candidate retains more cross-state linguistic variation and is evaluated separately. The excluded-instrument partial $F$ is `r report_value("partial_f_report")` with $p=$ `r report_value("partial_p_report")`. The first-stage coefficient on linguistic distance is `r report_value("first_stage_linguistic_distance_estimate")`. Conditional relevance is therefore extremely weak.

The second-stage EMI-exposure estimate is `r report_value("iv_emie_estimate_report")` with $p=$ `r report_value("iv_emie_p")`. Because the instrument contributes almost no conditional first-stage variation, this estimate should not be interpreted as a credible causal effect. The appropriate conclusion is not that EMI has no effect, but that this instrument cannot distinguish such an effect under the state-FE candidate specification.
