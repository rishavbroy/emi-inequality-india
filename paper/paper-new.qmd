...
execute:
  ...
  warning: false
  message: false
abstract: "English skills are economically valuable in India, but the opportunity to acquire and use them need not be equally available. We follow an inherited linguistic opportunity gradient across language behavior, schooling, and later local economic outcomes. Census language data show that linguistic distance from Hindi predicts language acquisition nonlinearly, but its translation into formal English-medium instruction (EMI) is strongly organized by state institutions. Public EMI is especially state-organized, while private EMI retains more local variation and is more socially unequal in access. On common district support, English-oriented schooling margins are more strongly associated with later household consumption than enrollment alone, particularly in 2022-23; these are descriptive associations, not causal returns. Linguistic distance cannot credibly isolate EMI within states: the first stage collapses after state absorption, alternative linguistic measures do not rescue it, and independent DISE data closely validate the NSS schooling measures. The same linguistic environment is also associated with selective human-capital, financial, migration, and service-sector development rather than broad employment expansion. We therefore interpret EMI as one socially selective institution within a broader opportunity environment, not as a standalone equalizing technology."
...
<!--
WORKING PAPER: ChatGPT-assisted edits belong in paper-new.qmd only.
The strict pipeline renders this draft alongside paper.qmd so every main-paper
exhibit and dependency is continuously validated. Application-sample excerpts
remain tied to paper.qmd until corresponding prose is manually approved.
-->
...
On the other hand, there are limitations unique to the many flaws of the Indian education system: low-effort [@singhSizeSourcesPrivate2013], even abusive [@centreforbudgetandpolicystudiesQualitySystemicFunctioning2020, pp. 218-22] teachers whose annual absenteeism rates have remained near 25% for decades [@muralidharanFiscalCostWeak2017; @chaudhury.etal2006, who also find that only 45% of teachers at any one point in time were engaged in any kind of teaching-related activity]^[The long-term consistency of this trend reflects the longevity of the civil service protections which produce it: limits on teacher hiring/firing power in government schools enable qualified teachers to extract such rents [@chaudhury.etal2006]. Even when rarely-absent teachers rationally advocate for such protections, they end up further deepening the culture of absence which @basuTeacherTruancyIndia2006 finds to explain the significant state-by-state heterogeneity in teacher absenteeism rates. The simple act of increasing the frequency of teacher monitoring, however, is enough to drastically reduce absenteeism [@kremerTeacherAbsenceIndia2005; @muralidharanFiscalCostWeak2017].] coupled with inefficient resource allocation [@muralidharanContractTeachersExperimental2013a; @gooptu2023a] and poor commuting infrastructure [@kremerTeacherAbsenceIndia2005], all alongside schools which actively lie about their medium of instruction [@mathew2011a; @lahoti2019a], all of which is more commonly found in government schools than private. And yet the mismatch between expectation and reality can be so great that, even among the rural *private* schools surveyed by @lahoti2019a, "more than half (57%) of the children who [were] supposed to be [in EMI were] actually studying in the dominant regional language" (p. 55). Navigating such an environment requires parents to have enough information and social capital with which to identify higher-quality schooling/tutoring^[The best evidence for this comes from @andrabiReportCardsImpact2017 and @afridiImprovingLearningOutcomes2020, whose experimental treatments of education information provisioned to parents led to a more efficient market and improved student outcomes, particularly in the private schools nudged to improve quality by the treatment.]---often the product of being well-educated themselves [@lahoti2019a; @blimpoParentalHumanCapital2015]---but also to have a high enough income to afford this higher-quality^[Which in practice often equals private schooling; see @tooleySchoolChoiceAcademic2011.] education [@muralidharan2008a; @singhSizeSourcesPrivate2013].
...
The evidence in this paper points to a broader question than whether one school-medium coefficient is positive:
\begin{quote}
When linguistic conditions make English relatively attractive, how is that opportunity translated into schooling and local economic gains, and who is positioned to capture it?
\end{quote}
The distinction matters because learning English is not the same object as learning through English. English skills can be valuable in the labor market even when unfamiliar-language instruction imposes educational costs, and the schooling system can ration access to English-oriented instruction along institutional and social lines.

We study this question at two linked levels. The individual NSS education records reveal who enters schooling and who reaches English-oriented and private schooling margins. The district analysis then harmonizes language, schooling, Census, administrative, and consumption data to Census-2001 districts. Because we do not observe a household panel, the long-run welfare comparisons are district pseudo-panel associations rather than household treatment effects.^[A district in India is akin to a county in the United States, but with a much larger average population. @tbl-core-summary summarizes the central district-level constructs used throughout the paper, and @sec-distma-spa documents the district-lineage construction.]

The empirical sequence follows the economic argument. We first ask whether inherited linguistic structure is associated with language choice. We then ask how strongly that opportunity gradient appears in formal schooling, how public and private EMI are geographically organized, and how access differs across social groups. Next we test whether the schooling margins households appear to value are descriptively associated with later welfare and whether predetermined local capacity strengthens those associations. Only then do we ask whether linguistic distance can isolate a causal EMI effect. It cannot: the first stage is absorbed by state geography and remains weak under alternative language constructions. That failure motivates the final reduced-form question—what broader pattern of local development accompanies the inherited linguistic-opportunity environment beyond measured EMI?

The original education-participation probit remains useful corroborating evidence on selection into schooling, but it is not the organizing empirical design. Likewise, conventional 2SLS is retained as an identification stress test rather than treated as the paper's headline estimator. The distinction is important throughout: schooling-welfare estimates are conditional district associations, and reduced-form language-development estimates describe co-evolving or later local outcomes rather than a sequential mediation mechanism.
...
render_public_table("../outputs/tables/main/sum_tbl_probit_quant.tex", "sum_tbl_probit_quant")
```
...
render_public_table("../outputs/tables/main/sum_tbl_probit_cat.tex", "sum_tbl_probit_cat")
```
...
We first look at household-level determinants of who gets to stay in school. Our data is of children aged 5-19, so as a 5-18 year old child grows one year older, the probability they stay enrolled in school decreases by `r report_value("ame_age_pct")` percentage points on average, indicative of either economic necessity, the opportunity cost of school, or even disillusionment with school quality growing in age.^[In 2002, the right to free and compulsory education for all children aged 6-14 was enshrined by @jainConstitutionEightysixthAmendment2002 into the Constitution of India. One might expect this to explain our $\mathrm{AME}_{\text{Age}}$ result as the product of a steep drop in enrollment between the ages of 14 and 15 which violates the linearity assumption underlying the interpretation of AME values [@scholbeckMarginalEffectsNonlinear2024]. But the amendment neither enforced nor implemented, instead only mandating future legislation to do so; such legislation was not passed until 2009 [@dwivediABCRTE2018, @departmentofhighereducationProvisionsConstitutionIndia2010]. Something which did exist in 2007-08, however, was the Sarva Shiksha Abhiyan program, which had been allocating funds for school construction, facilities management, teacher training, girls' education, and so on since 2001-02 [@pressinformationbureauInitiativesHumanResource2008]. One would expect this to increase (as in make more positive) our $\mathrm{AME}_{\text{Age}} =$ `r report_value("ame_age_pct")` estimate, and it likely has increased our $\mathrm{AME}_{\text{Female}} =$ `r report_value("ame_sex_pct")` estimate.] We also see that each additional household member lowers enrollment probability by `r report_value("ame_hh_size_pct")` percent. A negative value is expected---household resources should dilute as size increases---but the slight value and high significance are notable given the quantity-quality tradeoff of e.g., @beckerEconomicAnalysisFertility1969, plus empirical tests of it in India. @kumarTestingChildrenQuantityQuality2011 find that, by using children's educational attainment as a proxy for quality, cross-sectional dataset from 2007-08 indicates the tradeoff is heterogeneous in India, stronger given rural areas, low-caste children, illiterate mothers, and poor children. Conversely, by instrumenting family size  @onurBirthOrderSibling2021 remove the quantity-quality tradeoff entirely from their panel data, whether quality is proxied by educational expenditures on a child or test scores. Enrollment is such a relatively low-cost, extensive-margin measure for quality, however, that it makes sense that neither the heterogeneity nor non-existence of these previous studies shows up here. The significance of our coefficient is understandable given the many correlates of fertility and schooling amongst our regressors, whereas the small magnitude of our estimate may arise from a non-linearity in enrollment (e.g., if household size only begins to constrain at a high enough level), from economies of scale in enrollment costs, and from the aforementioned low quality threshold that is enrollment. This last factor also colors our interpretation of the relatively high probability of female enrollment (`r report_value("ame_sex_pct")` percent more than male enrollment): while it likely reflects the success of programs such as the Sarva Shiksha Abhiyan and the Kasturba Gandhi Balika Vidyalayas girls' schools [@pressinformationbureauInitiativesHumanResource2008], it also tells us nothing about the quality of education beyond enrollment.

While both Muslims and Christians are less likely to be enrolled than Hindus (whereas Buddhists and Zoroastrians are more likely), the sharp Muslim penalty of `r report_value("ame_muslim_pct")` percent echoes @borooah2021a, who find that even upper-class Muslims enroll at lower rates. Even greater statistical significance can be found among Scheduled Tribe, Scheduled Caste, and Other Backward Class members, all of whom have negative coefficients, with Scheduled Tribes faring the worst at `r report_value("ame_st_pct")` percent. While we should be suspicious of causal interpretations, it is likely these coefficients reflect the way structural factors i.e., those which cannot be changed through individual actions alone, constrain the choice set over which households optimize over. For example, if we use chronic teacher absenteeism to reflect school quality, then we can use @kremerTeacherAbsenceIndia2005 to see that better infrastructure and proximity to a paved road correlate with increased school quality, as does more frequent school monitoring per both Kremer et al. and @muralidharanFiscalCostWeak2017. If we assume that the physical and political infrastructure needed to do this is decreasing in the share of ST/SC/OBC members in the area, then this only decreases the opportunity cost of labor for children in these groups.

Looking deeper into the supply side of education, it seems that all variation which could potentially be explained by enrollment cost has, at best, been consumed by other variables, implying direct costs matter less than social barriers and other correlates of supply-side factors. While one would expect the causal effect of an exogenous shock in scholarships/stipends, stationery, or textbooks to match the sign of their positive coefficient estimates, only textbooks show up as statistically significant. Instead, our other significant result is the `r report_value("ame_edu_free_pct")` percent change in enrollment associated with a marginal change in the share of students within one's district who attend a school which charges no tuition fees (i.e., where 'Educ. freely available'). @nationalsamplesurveyoffice2008 documentation indicates that such schools include most if not all government schools, as well as private schools in some states up to a certain level of education. Building off observations in @sec-intro that government schools tend to have worse facilities, chronic teacher absenteeism, and so on, this coefficient seems to imply that when the modal inside option is poor-quality schooling, the marginal child is more likely to have taken the outside option not be enrolled. If this interpretation is correct, then comparing the variables' $s$-values^[Given a $p$-value, we can define the $s$-value as $s=-\log_2(p)$ [@mansourniaPvalueCompatibilitySvalue2022].] using @mansourniaPvalueCompatibilitySvalue2022 reveals just how much evidence we find for government schools being a negatively-priced deterrent [a la @muralidharan2019a]: while both 'Textbook(s) received' and 'Educ. free available' have similar effect sizes and standard errors, the former has an $s$-value of `r report_value("ame_textbooks_s")`, meaning that the data provided `r report_value("ame_textbooks_s")` bits of information against the null hypothesis (a coefficient of zero), whereas the 'Educ. free available' result communicated a far higher `r report_value("ame_edu_free_s")` bits against its null.
...
# From Linguistic Opportunity to Schooling and Welfare {#sec-iv}
## District-level objects and descriptive estimands {#sec-iv-model}
Inspired by measures of public-school exposure in @chakraborty2016a, we define district all-child EMI exposure (EMIE) as the survey-weighted share of children ages 5--19 who are both enrolled and observed in English-medium instruction:
$$\text{EMIE}_d = 100\times\frac{\text{Num. children ages 5--19 enrolled in EMI}}{\text{Num. children ages 5--19}}\,\,\, \text{in district }d.$$

This exposure combines the extensive margin of enrollment with medium choice among enrolled children. We therefore keep it distinct from enrollment, EMI conditional on enrollment, public EMI, and private EMI. The main welfare exercise asks whether these schooling margins are associated with later real mean household consumption on common district support, using predetermined Census-2001 controls and state fixed effects. Those estimates are descriptive conditional associations rather than causal returns.

Linguistic distance enters first as a predetermined characteristic of the local language environment. We later ask whether it can also serve as excluded variation for EMI. For that identification stress test, the candidate 2SLS system is
$$
\begin{aligned}
\mathrm{EMIE}_d
...
\end{aligned}
$$ {#eq-iv-eq}
where $X_d$ contains predetermined Census-2001 controls and $\alpha_s$ denotes Census-2001 state fixed effects. Because the conditional first stage is weak, this system defines the identification boundary rather than the preferred interpretation of the schooling-welfare evidence.
The scale and dispersion of the paper's central schooling, linguistic, predetermined-capacity, and long-run welfare constructs are summarized in @tbl-core-summary. @fig-schooling-geography places the preferred linguistic condition beside all-child, public, and private EMI exposure on the common Census-2001 district geography.
...
@tbl-schooling-market-geography collects the institutional-geography evidence that otherwise sits across several diagnostics. Linguistic distance is positively associated with every schooling margin nationally, but the standardized relationship attenuates sharply after region and especially state absorption. State membership explains much more of public EMI than private EMI variation. Finally, DISE and NSS EMI measures remain strongly correlated after state means are removed, so the attenuation is not plausibly explained by NSS measurement error alone.
...
The same district school market does not imply the same schooling opportunity set for every child. @fig-unequal-schooling-access summarizes the direct NSS-64 within-district gaps between Scheduled Tribe, Scheduled Caste, and Other Backward Class children and children in the residual Other group. Panel A compares the four schooling margins most relevant to the paper; Panel B shows the predeclared rural/urban cross-cut only for the two English-oriented margins where sectoral access is most informative. These are descriptive district gaps rather than causal effects or regression-adjusted estimates.
...
@tbl-language-behavior asks whether the linguistic-distance measure changes language behavior in the way the opportunity-cost story requires. The national continuous English-acquisition slope is positive but imprecise, whereas the predeclared distant-language contrast is much larger. Hindi acquisition moves in the opposite direction, and greater distance is also associated with lower multilingualism among native speakers. Inside the Hindi belt, however, neither English specification reproduces the national pattern cleanly. The evidence therefore supports a broader language-opportunity interpretation rather than a simple linear English-versus-Hindi substitution rule.
...
## Schooling and Later Welfare {#sec-iv-results}
@tbl-paper-schooling-welfare asks a narrower descriptive question: are the schooling margins emphasized in the access analysis associated with later district living standards on common support? The table compares enrollment, conditional EMI, all-child EMI, public EMI, and private EMI using the same state-fixed-effects and predetermined-control specification within each welfare endpoint. English-oriented margins are substantially more positively associated with 2022 consumption than enrollment alone, while the corresponding 2023 relationships are weaker. Because these are conditional district associations rather than experimentally or quasi-experimentally isolated returns, they motivate rather than resolve the identification question.

```{r}
#| label: tbl-paper-schooling-welfare
#| echo: false
#| results: asis

render_public_table("../outputs/tables/main/paper_schooling_welfare.tex", "paper_schooling_welfare")
```

@tbl-conversion-complements asks whether the descriptive schooling-welfare association is stronger where predetermined local capacity was greater. Human capital and urbanization strengthen the all-child EMI association, while ST concentration does not. Private-EMI interactions point in the same direction for human capital and urbanization but are less precisely estimated after family correction. The separate 2005 Economic Census check finds little evidence that baseline IT-employment intensity generally amplifies either the all-child EMI association or the reduced-form linguistic-distance relationship. The pattern is therefore one of selective complementarity rather than a generic high-IT-district explanation.

```{r}
#| label: tbl-conversion-complements
#| echo: false
#| results: asis

render_public_table("../outputs/tables/main/paper_conversion_complements.tex", "paper_conversion_complements")
```

The reduced form is more informative about the long-run timing pattern than the conventional 2SLS coefficient path because the first stage is weak at every horizon. @fig-consumption-reduced-form-dynamics therefore follows the coefficient on linguistic distance itself across the four registered post-treatment endpoints. It shows both endpoint ANCOVA and the corresponding long change from the common 2004-05 baseline rather than selecting one estimand after seeing the results. The early 2009-10 and 2011-12 estimates are small and imprecise, while the long-change association becomes positive by 2022-23 and remains positive but less precise in 2023-24. These are reduced-form associations, not estimates of an EMI effect.
...
## Identification Boundary: Why Linguistic Distance Does Not Isolate EMI {#sec-iv-iv}

Our preferred linguistic measure is the speaker-weighted mean Shastry distance from Hindi among mapped mother tongues with positive distance in district $d$ in 2001. Following @shastry2012a, as the linguistic cost of acquiring Hindi rises, English can become relatively more attractive as a bridge language. The C-17 evidence in @tbl-language-behavior supports a language-behavior response, but it also shows that the response is nonlinear and institutionally contingent. Relevance for formal EMI therefore has to be demonstrated rather than assumed.

Nationally, the relationship is visible. Within states, it nearly disappears. @fig-first-stage-absorption holds district support fixed while progressively adding geographic absorption and predetermined controls. The excluded-instrument F statistic is in the low-to-mid teens without fixed effects and falls to approximately one or below under state absorption. This is not a sample-composition artifact, and richer predetermined controls do not restore the signal.
...
@tbl-identification-boundary asks whether this conclusion depends on one particular linguistic-distance construction. It does not. The preferred Shastry mean, the legacy top-three measure, Glottolog and Dyen alternatives, the distant-language share, and a richer five-share language vector all lose most of their broad predictive content once the design is restricted to within-state comparisons. The richer language vector is jointly predictive in levels, but that fact does not supply persuasive within-state excluded variation.
...
The long-run 2022 and 2023 2SLS rows in Panel B make the inferential consequence explicit. Conventional coefficients are unstable relative to their standard errors, Montiel Olea--Pflueger effective F statistics remain far below conventional relevance benchmarks, and the Anderson--Rubin confidence sets are disconnected across positive and negative values. A finite-grid rejection of zero therefore does not identify the sign of an EMI effect. We consequently do **not** interpret the schooling-welfare associations as causal effects of EMI.

The measurement-validation evidence points in the same direction. NSS and DISE measures of EMI agree strongly in levels and remain strongly correlated after state residualization (@tbl-schooling-market-geography), so weak within-state relevance is not plausibly repaired by replacing one schooling dataset with the other. The useful conclusion is substantive rather than merely technical: inherited linguistic incentives and formal English-medium schooling are related objects, but their correspondence is heavily organized by state institutions and local schooling markets. Linguistic distance is therefore better treated as an inherited opportunity gradient than as a credible narrow EMI instrument.
...
Taken together, the evidence rejects a simple schooling-technology chain in which inherited linguistic distance mechanically generates EMI and EMI then generates district prosperity. Linguistic structure is associated with language behavior, but formal EMI is far more state-organized than general enrollment and is sharply stratified within local school markets. The schooling margins most strongly associated with later 2022 welfare are English-oriented margins rather than enrollment alone, especially private EMI, yet the historical language gradient contributes too little within-state first-stage variation to isolate those associations causally.

That failure is informative because it changes the object of interest. Linguistic distance appears to describe a broader opportunity environment whose relationship with formal schooling is filtered through institutions and social access. The positive conversion gradients for baseline human capital and urbanization, together with the null IT-baseline moderation result, further suggest that local complementary capacity matters without reducing the story to pre-existing technology clusters.

The reduced-form timing pattern reinforces this interpretation. Early consumption relationships are small and imprecise, while positive long-change associations emerge much later. This timing is consistent with slow-moving human-capital maturation, migration, and structural adjustment, but it does not identify which process causes the lag. The next section of the paper therefore needs to synthesize those broader development margins—including their nulls—rather than reinterpret weak 2SLS estimates as evidence for or against a causal EMI return.
...
The main paper now tests this complementarity argument directly rather than inferring it from the coefficients on controls. @tbl-conversion-complements predeclares interactions of all-child and private EMI with baseline human capital, urbanization, and ST concentration, and separately tests whether baseline Economic Census IT intensity moderates the schooling--welfare relationship. Human capital and urbanization amplify the all-child EMI association; ST concentration and baseline IT intensity do not provide a general explanation. These are descriptive heterogeneous associations, not causal mechanisms. Broader exploratory interactions therefore belong outside the main specification rather than being added after observing control coefficients.
