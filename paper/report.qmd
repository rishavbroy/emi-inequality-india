...
The preferred specification uses person-weighted real log consumption growth as the outcome, the predetermined Census 2001 control set, and Census-2001 state fixed effects. The excluded-instrument partial $F$ is `r report_value("partial_f_report")` with $p=$ `r report_value("partial_p_report")`. The first-stage coefficient on linguistic distance is `r report_value("first_stage_linguistic_distance_estimate")`. Conditional relevance is therefore extremely weak.

The second-stage EMI-exposure estimate is `r report_value("iv_emie_estimate_report")` with $p=$ `r report_value("iv_emie_p")`. Because the instrument contributes almost no conditional first-stage variation, this estimate should not be interpreted as a credible causal effect. The appropriate conclusion is not that EMI has no effect, but that this instrument cannot distinguish such an effect under the preferred control and fixed-effect specification.
