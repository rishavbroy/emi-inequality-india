...
![Trends in earnings, labor‐force participation, and unemployment (ILO, 2024).](../outputs/figures/main/fig_ilo_trends.png){#fig-ILO-fig}
...
Geospatial data intended for maps and spatial autocorrelation measures is sourced from @bhatiaMergingUpdatedDistrictlevel2020, which is itself an adaptation of @meyersIndiaOfficialBoundaries2020. The active figures below use district-level empirical distributions while the geometry join remains under validation. Our methods for tracking districts across time (see @sec-distma) begin with data from @indiastatestoriesDistrictEvolution2024 and @jaacksIndiaDistrictChanges2020.
...
#| tbl-cap: "Summary Statistics for 2SLS Model"
#| echo: false
#| results: asis
...
tex_path <- "../outputs/tables/main/sum_tbl_iv.tex"
if (knitr::is_latex_output() && file.exists(tex_path)) {
  cat(paste(readLines(tex_path, warn = FALSE), collapse = "\n"))
} else {
  print(knitr::kable(output_table("../outputs/tables/main/sum_tbl_iv.csv"), digits = 3))
}
```
...
tex_path <- "../outputs/tables/main/sum_tbl_probit_quant.tex"
if (knitr::is_latex_output() && file.exists(tex_path)) {
  cat(paste(readLines(tex_path, warn = FALSE), collapse = "\n"))
} else {
  print(knitr::kable(output_table("../outputs/tables/main/sum_tbl_probit_quant.csv"), digits = 3))
}
```
...
tex_path <- "../outputs/tables/main/sum_tbl_probit_cat.tex"
if (knitr::is_latex_output() && file.exists(tex_path)) {
  cat(paste(readLines(tex_path, warn = FALSE), collapse = "\n"))
} else {
  print(knitr::kable(output_table("../outputs/tables/main/sum_tbl_probit_cat.csv"), digits = 3))
}
```
...
#| tbl-cap: "Average Marginal Effects and Counterfactual Comparisons for Enrollment Probit"
#| echo: false
#| results: asis
output_table <- function(path) {
  if (file.exists(path)) return(utils::read.csv(path, check.names = FALSE))
...
tex_path <- "../outputs/tables/main/probit_mfx.tex"
if (knitr::is_latex_output() && file.exists(tex_path)) {
  cat(paste(readLines(tex_path, warn = FALSE), collapse = "\n"))
} else {
  print(knitr::kable(output_table("../outputs/tables/main/probit_mfx.csv"), digits = 3))
}
```
...
\end{aligned}
$$ {#eq-iv-eq}
Summary statistics for all of the variables in this model, including the controls $k$ in the vector $X_{kd}$, are provided in @tbl-sum-tbl-iv. Distribution figures for these variables are presented in @fig-map1-fig and @fig-map2-fig; the missing geometry join discussed in @sec-distma-spa is the product of a data harmonization method which performed many-to-many matching from 2001 to 2007-08 to 2017-18 to 2019-20, the years our shapefiles data was collected [@bhatiaMergingUpdatedDistrictlevel2020]. Issues of and improvements to this method are also discussed in @sec-distma-spa.
...
![From left to right: regions of India and linguistic distance from Hindi. District-level data, from the 2001 Census of India.](../outputs/figures/main/collage_iv_region_maps.png){#fig-map2-fig}
...
```{r}
#| label: tbl-fs-cons
#| tbl-cap: "First-Stage Regression: EMI Exposure on Linguistic Distance"
#| echo: false
#| results: asis
output_table <- function(path) {
  if (file.exists(path)) return(utils::read.csv(path, check.names = FALSE))
...
tex_path <- "../outputs/tables/main/fs_cons.tex"
if (knitr::is_latex_output() && file.exists(tex_path)) {
  cat(paste(readLines(tex_path, warn = FALSE), collapse = "\n"))
} else {
  print(knitr::kable(output_table("../outputs/tables/main/fs_cons.csv"), digits = 3))
}
```

```{r}
#| label: tbl-cons-iv
#| tbl-cap: "Second-Stage Regression: Consumption Growth on EMIE (Fitted)"
#| echo: false
#| results: asis
output_table <- function(path) {
  if (file.exists(path)) return(utils::read.csv(path, check.names = FALSE))
...
tex_path <- "../outputs/tables/main/cons_iv.tex"
if (knitr::is_latex_output() && file.exists(tex_path)) {
  cat(paste(readLines(tex_path, warn = FALSE), collapse = "\n"))
} else {
  print(knitr::kable(output_table("../outputs/tables/main/cons_iv.csv"), digits = 3))
}
```
...
![Number of 2001 districts which absorbed a percentage of a 1991 district's population via name change, clean merger, carve-out, or border shift. Data from Kumar \& Somanathan (2016).](../outputs/figures/main/district_carveouts_shifts.png){#fig-districtcarveoutsshifts-fig}
