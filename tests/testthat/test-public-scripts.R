...
test_that("analysis notebooks render through cached targets, not unconditional Quarto loops", {
  ...
  expect_match(targets, "analysis_note_targets <- analysis_markdown_target_definitions", fixed = TRUE)
  ...
  expect_match(renderer, "setwd(dirname(qmd))", fixed = TRUE)
  expect_match(renderer, "system2(\"quarto\", c(\"render\", basename(qmd), \"--to\", \"gfm\"))", fixed = TRUE)
