...
test_that("extended IV artifacts are reachable from the diag_ext audit selector", {
  ...
  lineage_targets <- paste(
    readLines(repo_file("R", "pipeline", "extended_lineage_targets.R"), warn = FALSE),
    collapse = "\n"
  )
  expect_match(
    lineage_targets,
...
test_that("working paper is rendered and checked by the strict publication graph", {
  ...
  required_markers <- c(
    ...
    "paper_unequal_schooling_access.pdf",
    "paper_language_behavior.tex",
    "paper_economic_conversion.tex",
    "consumption_iv_dynamics.pdf",
    "paper_local_development.tex",
...
test_that("paper local-development synthesis promotes only publication analysis objects", {
  targets <- repo_text("_targets.R")
  ...
  expect_match(targets, "core_census_development_target_definitions()", fixed = TRUE)
  expect_match(targets, "core_labor_target_definitions()", fixed = TRUE)
  expect_match(core_census, "census_migration_diagnostics", fixed = TRUE)
  expect_match(core_census, "census_housing_diagnostics", fixed = TRUE)
  expect_match(core_census, "census_household_capacity", fixed = TRUE)
  expect_false(grepl("tar_target(\n      census_migration_diagnostics,", extended_census, fixed = TRUE))
  expect_false(grepl("tar_target(\n      census_housing_diagnostics,", extended_census, fixed = TRUE))
  expect_false(grepl("tar_target(\n      census_household_capacity,", extended_census, fixed = TRUE))
  expect_match(extended_census, "save_census_migration_diagnostics(census_migration_diagnostics)", fixed = TRUE)
  expect_match(extended_census, "save_census_housing_diagnostics(census_housing_diagnostics)", fixed = TRUE)
  expect_match(extended_census, "save_census_household_capacity(census_household_capacity)", fixed = TRUE)

  expect_match(core_ec, "economic_census_diagnostics", fixed = TRUE)
  expect_false(grepl("tar_target(\n      economic_census_diagnostics,", extended_ec, fixed = TRUE))
  expect_match(extended_ec, "save_economic_census_diagnostics(economic_census_diagnostics)", fixed = TRUE)

  expect_match(core_labor, "nss66_labor_mechanism", fixed = TRUE)
  expect_match(core_labor, "plfs_2017_18_labor_mechanism", fixed = TRUE)
  expect_false(grepl("tar_target(\n      nss66_labor_mechanism,", extended_labor, fixed = TRUE))
  expect_false(grepl("tar_target(\n      plfs_2017_18_labor_mechanism,", extended_labor, fixed = TRUE))
  expect_match(extended_labor, "save_labor_mechanism_inference(nss66_labor_mechanism", fixed = TRUE)
  expect_match(extended_labor, "save_labor_mechanism_inference(plfs_2017_18_labor_mechanism", fixed = TRUE)

  expect_match(core_public, "paper_local_development", fixed = TRUE)
  expect_match(core_public, "out$paper_local_development", fixed = TRUE)
  expect_match(paper_new, "paper_local_development.tex", fixed = TRUE)
  expect_match(paper_new, "tbl-local-development", fixed = TRUE)
