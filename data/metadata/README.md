...
Census housing diagnostics activate 2011 HL-13 for follow-up structural durability. Census-2001 H-04 Appendix (`PC01_H04a`) is registered in the acquisition manifest as `H04A` but remains acquisition-only until the raw workbooks are inspected and their household denominator is reconciled to an established 2001 housing source. Durability changes are constructed only after that source-level validation.
