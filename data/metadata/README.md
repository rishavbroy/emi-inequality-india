...
The 1991 acquisition manifest is validation-oriented rather than production-active. It registers the reviewed official B-01(S), C-02 total/urban, C-06, and C-09 workbooks under `data/raw/census_1991/`. B-01(S), C-02, and C-06 are state workbooks containing district observations; C-09 is one all-India workbook containing district religion rows. C-02U is retained as a supplementary urban-education check. ORGI H-4/H-5 are not registered because the catalog exposes them only at country/state granularity, not district granularity. Downloaded tables do not enter any model until a reader, denominator contract, geography reconciliation, and explicit target are added.
