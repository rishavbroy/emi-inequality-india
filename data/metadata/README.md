...
- `checksums.csv`: checksums for every tracked metadata CSV/TSV. Generated processed outputs are validated by pipeline/tests rather than pinned to a pre-build MD5, because incremental builds may legitimately regenerate them. Run `Rscript scripts/update_checksums.R` after intentionally changing any of those files; the test suite verifies both inventory completeness and current MD5 values.
