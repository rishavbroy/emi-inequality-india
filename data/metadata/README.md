...
- `map_disputed_areas.csv`: explicit allowlist for display-only disputed/no-district-estimate polygons. It records the Natural Earth `BRK_NAME` used for exact selection and a paper-facing label; it must not be inferred from broad country-name or claim-line filters. Public maps union those polygons with the processed DataMeet 99/99 Jammu-and-Kashmir scaffold and subtract the same display mask from rendered canonical districts. Neither source changes the Census-2001 analytical panel.
