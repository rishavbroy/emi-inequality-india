...
census_c17_mechanism_registry <- function() {
  data.frame(
    ...
    outcome = c(
      rep("english_share_multilingual", 5L),
    ...
    distance_variable = c(
      rep("shastry_degree", 3L),
      "glottolog_edge_distance", "dyen_noncognate_pct",
      rep("shastry_degree", 4L)
    ),
    distance_form = c("linear", "bins", "distant", rep("linear", 6L)),
    preferred = c(TRUE, rep(FALSE, 8L)),
    label = c(
      ...
      "English acquisition: Glottolog distance",
      "English acquisition: Dyen noncognate distance",
...
prepare_census_c17_mechanism_data <- function(
  ...
  out$glottolog_edge_distance <- NA_real_
  out$glottolog_family_id <- NA_character_
  if (!is.null(glottolog) && !is.null(glottolog_crosswalk)) {
    out <- attach_exact_label_glottolog_distance(out, glottolog, glottolog_crosswalk)
  }
  out$dyen_noncognate_pct <- NA_real_
  if (!is.null(historical_linguistics)) {
    out <- attach_dyen_language_distance(out, historical_linguistics, lexical_index)
  }
...
census_c17_mechanism_formula <- function(specification) {
  ...
  variable <- as.character(specification$distance_variable[[1L]])
  distance_terms <- switch(
    form,
    linear = c(variable, "hindi_urdu_reference"),
    bins = "distance_bin",
    distant = c("distance_distant", "hindi_urdu_reference"),
...
census_c17_distance_term <- function(term, specification) {
  variable <- as.character(specification$distance_variable[[1L]])
  startsWith(term, variable) |
    startsWith(term, "hindi_urdu_reference") |
...
fit_census_c17_mechanism <- function(data, specification) {
  ...
  distance_variable <- as.character(specification$distance_variable[[1L]])
  if (!distance_variable %in% names(data)) {
  ...
  if (nrow(sample) < 10L || length(unique(sample$state_code)) < 2L ||
    ...
    return(list(
      ...
      summary = data.frame(
        ...
        r.squared = NA_real_, adjusted.r.squared = NA_real_,
...
summarize_census_c17_mapping_coverage <- function(data) {
  data <- safe_df(data)
  basis_variables <- c(
    shastry = "shastry_degree",
    glottolog = "glottolog_edge_distance",
    dyen = "dyen_noncognate_pct"
  )
  safe_bind_rows(lapply(names(basis_variables), function(basis) {
    variable <- basis_variables[[basis]]
    ...
    safe_bind_rows(lapply(names(groups), function(sex) {
      ...
      data.frame(
        ...
        n_mapped_cells_without_multilinguals = sum(
          mapped & num(part$multilingual_speakers) <= 0, na.rm = TRUE
        ),
        stringsAsFactors = FALSE
      )
    }))
