...
natural_earth_map_reference_paths <- function(paths = build_paths()) {
  ...
  c(
    countries = file.path(base, "ne_10m_admin_0_countries.shp"),
    disputed_areas = file.path(base, "ne_10m_admin_0_disputed_areas.shp"),
    disputed_lines = file.path(base, "ne_10m_admin_0_boundary_lines_disputed_areas.shp")
  )
}
...
read_natural_earth_map_reference <- function(files) {
  ...
  required <- c("countries", "disputed_areas", "disputed_lines")
  if (!all(required %in% names(files))) {
    stop("Natural Earth boundary reference requires countries, disputed areas, and disputed lines.", call. = FALSE)
  }
  ...
  countries <- sf::st_read(files[["countries"]], quiet = TRUE, stringsAsFactors = FALSE)
  disputed_areas <- sf::st_read(files[["disputed_areas"]], quiet = TRUE, stringsAsFactors = FALSE)
  disputed_lines <- sf::st_read(files[["disputed_lines"]], quiet = TRUE, stringsAsFactors = FALSE)
