...
#' Render a paper PDF
#'
#' @param paper_qmd Path to the paper QMD.
...
render_paper_pdf <- function(paper_qmd, report_values, figure_files, table_files) {
  ...
  if (!file.exists(paper_qmd)) {
    stop("Paper source QMD does not exist: ", paper_qmd, call. = FALSE)
  }
  if (!nzchar(Sys.which("quarto"))) {
    stop("Quarto CLI was not found on PATH; cannot render ", paper_qmd, call. = FALSE)
  }

  pdf_path <- file.path(dirname(paper_qmd), paste0(tools::file_path_sans_ext(basename(paper_qmd)), ".pdf"))
  status <- system2("quarto", c("render", paper_qmd, "--to", "pdf"))
  if (!identical(status, 0L)) {
    stop("quarto render ", paper_qmd, " --to pdf failed with status ", status, call. = FALSE)
  }
