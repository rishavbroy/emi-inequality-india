...
paper_first_stage_absorption_registry <- function() {
  data.frame(
    specification_id = c(
      "instrument_only",
      "census_controls",
    ...
    label = c(
      ...
      "Region FE",
      "Region FE + controls",
      "Region FE + expanded controls",
      "State FE",
      "State FE + controls",
      "State FE + expanded controls"
    ),
    stringsAsFactors = FALSE
  )
}

paper_first_stage_absorption_plot_data <- function(diagnostics) {
  summary <- safe_df(diagnostics$semantic_summary %||% data.frame())
  required <- c(
    "semantic_specification_id", "excluded_instrument_f",
    "partial_r_squared", "status"
  )
  missing <- setdiff(required, names(summary))
  if (length(missing)) {
    stop(
      "Paper first-stage absorption figure input is missing columns: ",
  ...
  summary <- summary[
    summary$semantic_specification_id %in% registry$specification_id,
    required, drop = FALSE
  ]
  if (nrow(summary) != nrow(registry) ||
      anyDuplicated(summary$semantic_specification_id) ||
      !setequal(summary$semantic_specification_id, registry$specification_id)) {
    stop(
      "Paper first-stage absorption figure requires the registered nine-specification ladder.",
      call. = FALSE
    )
  }
  if (any(summary$status != "estimated")) {
    stop("Paper first-stage absorption ladder contains a non-estimated specification.", call. = FALSE)
  }

  summary <- summary[
    match(registry$specification_id, summary$semantic_specification_id),
    , drop = FALSE
  ]
  summary$specification <- factor(
    registry$label, levels = rev(registry$label)
  )
  summary$excluded_instrument_f <- num(summary$excluded_instrument_f)
  summary$partial_r_squared <- num(summary$partial_r_squared)
  if (any(!is.finite(summary$excluded_instrument_f)) ||
      any(!is.finite(summary$partial_r_squared))) {
    stop("Paper first-stage absorption ladder requires finite strength diagnostics.", call. = FALSE)
  }
  summary$partial_r2_label <- sprintf("partial R2 = %.3f", summary$partial_r_squared)
  rownames(summary) <- NULL
  summary
}

save_first_stage_absorption_figure <- function(
    spec, path_base, formats, diagnostics) {
  need_pkg("ggplot2", "paper first-stage absorption figure")
  plot_data <- paper_first_stage_absorption_plot_data(diagnostics)

  p <- ggplot2::ggplot(
    plot_data,
    ggplot2::aes(x = excluded_instrument_f, y = specification)
  ) +
    ggplot2::geom_segment(
      ggplot2::aes(x = 0, xend = excluded_instrument_f, yend = specification),
      linewidth = 0.45
    ) +
    ggplot2::geom_point(size = 2.5) +
    ggplot2::geom_text(
      ggplot2::aes(label = partial_r2_label),
      hjust = -0.08, size = 3.1
    ) +
    ...
    ggplot2::labs(
      title = spec$title,
      subtitle = spec$subtitle,
      x = "State-clustered excluded-instrument F",
      y = NULL,
      caption = paste(
        "All specifications use one complete district sample and the preferred linguistic-distance instrument.",
        "Partial R2 is the squared residual correlation after the displayed adjustment.",
        "Effective-F diagnostics are reported separately because they are not the statistic plotted in this ladder."
      )
    ) +
    ggplot2::theme_minimal(base_size = 11) +
    ggplot2::theme(
  ...
  save_plot_formats(p, path_base, formats, width = 8.2, height = 5.2, dpi = 300)
}
...
save_figures <- function(figures, cfg) {
  ...
  for (name in names(figures)) {
    ...
    paths <- switch(
      ...
      first_stage_absorption = save_first_stage_absorption_figure(
        spec, path_base, formats,
        attr(figures, "first_stage_absorption")
      ),
