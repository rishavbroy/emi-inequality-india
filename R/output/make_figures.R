...
make_figures <- function(district_panel, raw_ilo_figures, cfg, iv_models = NULL, map_geometry = NULL) {
  ...
  map_specs <- list(
    ...
    map_consumption_growth = figure_spec("map_consumption_growth", "map_consumption_growth.png", "Change in Log Real Consumption", kind = if (maps_available) "map" else "status", variable = "real_log_consumption_change"),
