...
spatial_diagnostic_value <- function(
    diag, legacy_name = NULL, pattern = NULL, field = "p.value",
    contiguity = "rook", digits = 3L) {
  ...
  if (!is.null(contiguity) && "contiguity" %in% names(x)) {
    x <- x[as.character(x$contiguity) == contiguity, , drop = FALSE]
  }
  if (!is.null(legacy_name) && "legacy_name" %in% names(x)) {
    exact <- x[as.character(x$legacy_name) == legacy_name, , drop = FALSE]
    out <- first_available_number(exact, field)
    if (is.finite(out)) return(signif(out, digits))
  }
  if (!is.null(pattern)) {
    text_cols <- intersect(
      c("legacy_name", "estimand", "model", "target", "outcome", "name", "source", "variable", "test"),
      names(x)
    )
    if (length(text_cols)) {
