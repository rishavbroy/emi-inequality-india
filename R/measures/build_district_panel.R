...
analysis_panel_validation_failures <- function(out) {
  ...
  general_validation <- district_panel_blocking_issues(
    attr(out, "district_panel_validation", exact = TRUE)
  )
  if ("message" %in% names(general_validation) && nrow(general_validation)) {
    failures <- c(
      failures,
      unique(stats::na.omit(as.character(general_validation$message)))
    )
  }
