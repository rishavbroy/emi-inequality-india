...
For `read_sav_short()`, the legacy code used `shell()` on Windows to ask
`cmd.exe` for the 8.3 alias of a file’s absolute path, opened a binary
connection with `file(short_path, "rb")`, and passed that connection to
...
Registry, readr/vroom, and Excel-reader internals. The rendered note now
preserves those notes as prose while showing the current production
reader contract from R/io/read_long_paths.R rather than copying the full
legacy implementation into the analysis notebook.
