...
These diagnostics preserve additive administrative counts through Census-2001 harmonization and construct ratios only afterward. The 2005-06 and 2006-07 associations are predetermined school-system diagnostics estimated with state fixed effects and state-clustered inference. Dynamic coefficients use the preferred scalar linguistic-distance measure and the same two fixed-effect structures as the EMI trajectory, with 2007-08 retained only as the event-study reference year. The joint summaries separate pre-reference and post-reference distance×year tests, because the full-period omnibus statistic alone cannot distinguish pre-existing gradient changes from later divergence.
