...
The 2005-06 and 2006-07 associations are predetermined school-system diagnostics estimated with state fixed effects and state-clustered inference after count-preserving Census-2001 harmonization. Later raw School/Teacher sheets are not currently used for trajectories: comparison with the published district report cards reveals district-row alignment failures and cross-year field-definition discontinuities. The dynamic registry therefore records a deferred-reconstruction status rather than reporting coefficients from a non-comparable panel.
...
analysis_table(
  ...
  "School-quality trajectories: reconstruction status",
  max_rows = 10
)
analysis_table(
  dise_school_quality_event,
  "School-quality trajectories: coefficients (empty until reconstruction)",
  max_rows = 40
)
```
