...
Potential chi-square tests for independence: ANY NA vs. state; ANY NA vs region. The target output now renders those tests when the active cleaned data expose enough variation.
...
The current active data contain `r analysis_format_number(analysis_metric(missing_counts, "missing_var", "Total probit-model with NA", "n_missing"), 0)` probit-model rows with at least one missing value and `r analysis_format_number(analysis_metric(missing_counts, "missing_var", "Total probit-model with no NA", "n_missing"), 0)` probit-model rows with no missing value. Enrolled-only expenditure fields are diagnosed separately rather than included in this probit-model total.
...
```{r missingness-case-study-and-chisq}
