...
The legacy Chunk 20 diagnosed district-matching errors after
`merge_dfs_into_tracker()`, extracted unmatched rows, tagged their
source, compared tracker alternatives, created an all-rows search table
for close-match inspection, and asked why complete-case row counts
differed so much across flagged and joined objects.
